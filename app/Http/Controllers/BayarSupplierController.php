...
class BayarSupplierController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $payableAccount = $payableCode !== ''
            ...
            : null;
        $returnPayableCode = trim((string) DB::table('set_account')->where('faccount_name', 'RETBELIBLMPOTHUTANG')->value('faccount'));
        $returnPayableAccount = $returnPayableCode !== ''
        ...
        DB::transaction(function () use ($validated, $supplier, $headerAccount, $detailRows, $payableAccount, $returnPayableAccount, $adminAccount, $bankAdminFee, $voucherNo, $totalKasKeluar, $now, $references) {
            ...
            foreach ($detailRows->values() as $index => $row) {
                ...
                $journalDetailAmount = abs($paymentAmount);
                ...
                $referenceName = $supplier->fsuppliername;
                if ($refSupplierCode !== '' && str_starts_with(strtoupper($refSupplierCode), 'CU')) {
                    $customer = DB::table('mscustomer')->where('fcustomercode', $refSupplierCode)->first();
                    if ($customer) {
                        $subAccount = $customer->fcustomerid;
                        $referenceName = trim((string) $customer->fcustomername) ?: $referenceName;
                    }
                }
                $detailNote = $isReb ? $supplier->fsuppliername : trim($refNo . ' (' . $referenceName . ')');
                ...
                Trkasdt::create([
                    ...
                    'fkasmtno' => $voucherNo,
                    'ftrancode' => self::TRAN_CODE,
                    'faccount' => $detailAccount?->faccount,
                    'fdk' => $fdkDetail,
                    'frefno' => $row['frefno'],
                    'fnote' => $detailNote,
                    ...
                    'fjurnal' => $journalDetailAmount,
                    'fjurnal_rp' => $journalDetailAmount,
                    'fuserid' => $this->currentUserId(),
                    'fdatetime' => $now,
                    'fnou' => $index + 1,
    ...
    public function update(Request $request, $fkasmtno)
    {
        ...
        DB::transaction(function () use ($validated, $supplier, $headerAccount, $detailRows, $payableAccount, $returnPayableAccount, $adminAccount, $bankAdminFee, $voucherNo, $totalKasKeluar, $header, $references, $userIdLog) {
            ...
            if ($bankAdminFee > 0 && $adminAccount) {
                $createdAdminDetail = Trkasdt::create([
                    'fkasdtid'    => $nextDetailId + $detailRows->count(),
                    'fkasmtid'    => $header->fkasmtid,
                    'fkasmtno'    => $voucherNo,
                    'ftrancode'   => self::TRAN_CODE,
                ...
                // INSERT Log Detail Admin (Update)
                DB::table('log_trkasdt')->insert([
                    'ftrxlogid'    => $trxLogId,
                    'fkasdtid'     => $createdAdminDetail->fkasdtid,
                    'fkasmtno'     => $updatedHeader->fkasmtno,
                    'ftrancode'    => $createdAdminDetail->ftrancode,
                    'fnou'         => $createdAdminDetail->fnou,
                    'freftype'     => $createdAdminDetail->freftype,
                    'faccount'     => $createdAdminDetail->faccount,
                    'fsubaccount'  => $createdAdminDetail->fsubaccount,
                    'fdk'          => $createdAdminDetail->fdk,
                    'frefno'       => $createdAdminDetail->frefno,
                    'fnote'        => $createdAdminDetail->fnote,
                    'fdiscpersen'  => $createdAdminDetail->fdiscpersen,
                    'fdiscount'    => $createdAdminDetail->fdiscount,
                    'fkasdtvalue'  => $createdAdminDetail->fkasdtvalue,
                    'fvalue_rp'    => $createdAdminDetail->fvalue_rp,
                    'fjurnal'      => $createdAdminDetail->fjurnal,
                    'fjurnal_rp'   => $createdAdminDetail->fjurnal_rp,
                    'fselisihkurs' => $createdAdminDetail->fselisihkurs,
                    'fdiscountrp'  => $createdAdminDetail->fdiscountrp,
                    'fuserid'      => $createdAdminDetail->fuserid,
                    'fdatetime'    => $createdAdminDetail->fdatetime,
                    'feditmode'    => 'U',
                    'fuseridlog'   => $userIdLog,
                    'fdatetimelog' => $now,
                ]);
    ...
    private function normalizeDetails(array $details): Collection
    {
        return collect($details)
            ->map(function(array $detail) {
                ...
                $kasdtValue = round(abs($rawKasdtValue), 2);
    ...
    private function validateRemainingDoesNotExceedInvoice(Collection $detailRows): void
    {
        foreach ($detailRows as $index => $row) {
            $invoiceAmount = round(abs((float) ($row['fnilai_order'] ?? 0)), 2);
            $remainingAmount = round(abs((float) ($row['fsisa_hutang'] ?? 0)), 2);

            if ($invoiceAmount > 0 && $remainingAmount > $invoiceAmount) {
                throw ValidationException::withMessages([
                    "details.{$index}.fsisa_hutang" => 'Sisa hutang tidak boleh melebihi nilai nota.',
                ]);
            }
        }
    }
        ...
        if ($header) {
            $detailRows = $header->details
                ...
                ->map(function ($detail, $index) use ($referenceRemainMap) {
                    ...
                    $trCode = strtoupper(trim((string) ($reference->fstockmtcode ?? $detail->freftype ?? 'BUY')));
