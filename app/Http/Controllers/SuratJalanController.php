...
class SuratJalanController extends Controller
{
    ...
    public function pickableTer(Request $request)
    {
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->where('trstockmt.fstockmtno', 'ilike', "%{$search}%")
                    ...
                    ->orWhereRaw("TO_CHAR(trstockmt.fstockmtdate, 'YYYY-MM-DD HH24:MI:SS') ILIKE ?", ["%{$search}%"]);
            });
        }
        ...
        $sortableColumns = [
            'fstockmtno_display' => 'trstockmt.fstockmtno',
            'frefpo' => 'trstockmt.frefpo',
            'fsuppliername' => 'mssupplier.fsuppliername',
            'fstockmtdate' => 'trstockmt.fstockmtdate',
        ];

        $orderColumn = (string) $request->input('order_column', 'fstockmtdate');
        $orderDir = strtolower((string) $request->input('order_dir', 'desc')) === 'asc' ? 'asc' : 'desc';

        $query->orderBy($sortableColumns[$orderColumn] ?? 'trstockmt.fstockmtdate', $orderDir)
            ->orderBy('trstockmt.fstockmtid', 'desc');
        ...
        $rows = $query
            ...
            ->map(function ($row) {
                return [
                    'fstockmtid' => $row->fstockmtid,
                    'fstockmtno_display' => $this->formatDisplayTransactionNumber($row->fstockmtno ?? null, false),
                    'frefpo' => trim((string) ($row->frefpo ?? '')),
                    'fsuppliername' => trim((string) ($row->fsuppliername ?? '')),
                    'fstockmtdate' => $row->fstockmtdate ? Carbon::parse($row->fstockmtdate)->format('Y-m-d H:i:s') : null,
                ];
            });
    ...
    public function itemsTer($id)
    {
        $header = DB::table('trstockmt')
            ->leftJoin('mssupplier', 'mssupplier.fsuppliercode', '=', 'trstockmt.fsupplier')
            ->where('trstockmt.fstockmtid', $id)
            ->where('trstockmt.fstockmtcode', 'TER')
            ->where('trstockmt.fprdout', '0')
            ->select('trstockmt.*', 'mssupplier.fsuppliername')
            ->first();

        if (! $header) {
            return response()->json(['message' => 'Data penerimaan barang tidak ada.'], 404);
        }

        $usageSub = DB::table('trstockdt as srj_use')
            ->selectRaw("
                srj_use.frefdtno,
                srj_use.fprdcode,
        ...
        $items = DB::table('trstockdt')
            ...
            ->select([
                ...
                'trstockdt.fketdt',
                'trstockdt.frefdtno',
                DB::raw("COALESCE(trstockdt.fnoacak::text, '') as fnoacak"),
                DB::raw("COALESCE(trstockdt.frefnoacak::text, '') as frefnoacak"),
                DB::raw('COALESCE(trstockdt.fqtykecil, 0) as fqtykecil'),
                DB::raw('COALESCE(srj.fqtykecilkeluar, 0) as fqtykecilkeluar'),
                DB::raw("COALESCE(
                    CASE
                        WHEN trstockdt.fsatuan = m.fsatuanbesar
                            THEN (COALESCE(trstockdt.fqtykecil, 0) - COALESCE(srj.fqtykecilkeluar, 0)) / NULLIF(m.fqtykecil, 0)
                        WHEN trstockdt.fsatuan = m.fsatuanbesar2
                            THEN (COALESCE(trstockdt.fqtykecil, 0) - COALESCE(srj.fqtykecilkeluar, 0)) / NULLIF(m.fqtykecil2, 0)
                        ELSE COALESCE(trstockdt.fqtykecil, 0) - COALESCE(srj.fqtykecilkeluar, 0)
                    END, 0) as fqtyremain"),
            ])
            ->orderBy('trstockdt.fstockdtid')
            ->get()
            ->map(function ($item) use ($header) {
                $remainingQty = max(0, (float) ($item->fqtyremain ?? 0));
                $item->fqty = $remainingQty;
                $item->fqtyremain = $remainingQty;
                $item->maxqty = $remainingQty;
                $item->ftotal = round($remainingQty * (float) ($item->fprice ?? 0), 2);
                $item->frefpr = trim((string) ($item->frefdtno ?? $header->frefpo ?? ''));
