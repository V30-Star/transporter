...
class JurnalPembelianController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pageTitle = 'Jurnal Faktur Pembelian';

        if ($request->ajax()) {
            $query = DB::table('jurnalmt')->where('fjurnaltype', self::PURCHASE_JOURNAL_TYPE);
            $totalRecords = DB::table('jurnalmt')->where('fjurnaltype', self::PURCHASE_JOURNAL_TYPE)->count();
            ...
            $data = $records->map(function ($row) {
                ...
                $viewUrl = route('jurnalpembelian.view', $row->fjurnalmtid);
                ...
                $editUrl = route('jurnalpembelian.edit', $row->fjurnalmtid);
                ...
                $deleteUrl = route('jurnalpembelian.delete', $row->fjurnalmtid);
        ...
        return view('jurnalpembelian.index', compact(
            'canCreate',
            'canEdit',
            'canDelete',
            'showActionsColumn',
    ...
    public function create()
    {
        ...
        $newtr_prh_code = $this->generatePurchaseJournalNumber(now(), $fbranchcode);
        ...
        return view('jurnalpembelian.create', [
            ...
            'pageTitle' => 'Jurnal Faktur Pembelian',
            'fixedJournalType' => self::PURCHASE_JOURNAL_TYPE,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $fjurnaltype = self::PURCHASE_JOURNAL_TYPE;
        ...
        DB::transaction(function () use (
            ...
            if (empty($fjurnalno)) {
                $fjurnalno = $this->generatePurchaseJournalNumber($fjurnaldate, $kodeCabang);
            }
        ...
        return redirect()
            ->route('jurnalpembelian.index')
            ->with('success', 'Jurnal faktur pembelian berhasil disimpan.');
    }
    ...
    public function edit($fstockmtid)
    {
        ...
        return view('jurnalpembelian.edit', [
            'supplier' => $supplier,
            ...
            'famountpo' => 0,
            'action' => 'edit',
            'pageTitle' => 'Edit Jurnal Faktur Pembelian',
    ...
    public function view($fstockmtid)
    {
        ...
        return view('jurnalpembelian.view', [
            'supplier' => $supplier,
            'selectedSupplierCode' => $selectedSupplierCode,
            'fcabang' => $fcabang,
            'accounts' => $accounts,
            ...
            'famountponet' => 0,
            'famountpo' => 0,
            'indexRouteName' => 'jurnalpembelian.index',
            'editRouteName' => 'jurnalpembelian.edit',
        ]);
    }
    ...
    public function update(Request $request, $fstockmtid)
    {
        ...
        return redirect()
            ->route('jurnalpembelian.view', $fstockmtid)
            ->with('success', 'Jurnal faktur pembelian '.trim((string) $header->fjurnalno).' berhasil diperbarui.');
    }
    ...
    public function delete($fstockmtid)
    {
        ...
        return view('jurnalpembelian.delete', [
            'supplier' => $supplier,
            'selectedSupplierCode' => $selectedSupplierCode,
            'fcabang' => $fcabang,
            'accounts' => $accounts,
            ...
            'famountpo' => 0,
            'action' => 'delete',
            'pageTitle' => 'Hapus Jurnal Faktur Pembelian',
            'lockJournalType' => true,
        ]);
    }
    ...
    public function destroy($fstockmtid)
    {
        try {
            ...
            $message = 'Data jurnal faktur pembelian '.trim((string) $jurnaltransaksi->fjurnalno).' berhasil dihapus.';

            if (request()->expectsJson()) {
                return response()->json([
                    'message' => $message,
                    'redirect' => route('jurnalpembelian.index'),
                ]);
            }

            return redirect()->route('jurnalpembelian.index')->with('success', $message);
        } catch (\Exception $e) {
            ...
            return redirect()->route('jurnalpembelian.delete', $fstockmtid)->with('error', $message);
        }
    }
    ...
    private function generatePurchaseJournalNumber(Carbon $journalDate, ?string $branchCode): string
    {
        $branchCode = trim((string) ($branchCode ?? '')) ?: 'NA';
        $prefix = sprintf('%s.%s.%s.%s.', self::PURCHASE_JOURNAL_TYPE, $branchCode, $journalDate->format('y'), $journalDate->format('m'));
        $lockKey = crc32('JURNAL|'.self::PURCHASE_JOURNAL_TYPE.'|'.$branchCode.'|'.$journalDate->format('Y-m'));
        DB::statement('SELECT pg_advisory_xact_lock(?)', [$lockKey]);
        ...
        return $prefix.str_pad((string) ((int) $lastNo + 1), 4, '0', STR_PAD_LEFT);
    }
}
