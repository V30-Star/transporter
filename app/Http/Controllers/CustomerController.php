...
class CustomerController extends Controller
{
        ...
        try {
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'recordsFiltered' => 0,
                    'data' => [],
                    'error' => 'Gagal memuat data customer.',
                ], 500);
            }

            return redirect()
                ->route('dashboard')
                ->with('error', 'Gagal memuat data customer.');
        }
    }
    ...
    public function create()
    {
        ...
        try {
            ...
            return redirect()
                ...
                ->with('error', 'Gagal membuka halaman tambah customer.');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'frekening' => '',
                'fmemo' => '',
                'fnpwp' => 'nullable|string|prohibits:fnik',
                'fnik' => 'nullable|string|prohibits:fnpwp',
                ...
                'fnpwp.prohibits' => 'Hanya boleh isi salah satu: NPWP atau NIK.',
                'fnik.prohibits' => 'Hanya boleh isi salah satu: NPWP atau NIK.',
            ]);
    ...
    public function destroy($fcustomerid)
    {
        ...
        try {
            ...
            return redirect()
                ->route('customer.index')
                ->with('success', 'Customer ' . $customer->fcustomername . ' berhasil dihapus.');
        } catch (\Throwable $e) {
            if (request()->expectsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Customer belum bisa dihapus: ' . $e->getMessage(),
                ], 500);
            }

            return redirect()
                ->route('customer.index')
                ->with('error', 'Customer belum bisa dihapus: ' . $e->getMessage());
        }
    }
