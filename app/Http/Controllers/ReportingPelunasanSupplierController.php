...
class ReportingPelunasanSupplierController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reportingpelunasansupplier.index', [
            'hasFilter' => $hasFilter,
            'dateFrom' => $dateFrom,
            'dateTo' => $dateTo,
            'branches' => $branches,
    ...
    public function print(Request $request)
    {
        ...
        return view('reportingpelunasansupplier.print', [
    ...
    public function exportExcel(Request $request)
    {
        ...
        $filename = 'Laporan_Pelunasan_Supplier_' . now()->format('Ymd_His') . '.xlsx';
    ...
    private function detailQuery(array $filters)
    {
        $query = DB::table('trkasmt as m')
            ...
            ->whereRaw("TRIM(COALESCE(d.freftype, '')) IN ('PBL', 'REB', 'RUB')")
            ->selectRaw("
                m.fkasmtid,
                m.fkasmtno,
                d.freftype,
                ...
                m.famountpay_rp,
                m.fuserid,
                COALESCE(n.famountremain, 0) AS famountremain,
                n.fstockmtdate AS fdate_ref,
                n.famountmt AS famountso,
                CAST(0 AS numeric) AS fongkosangkut,
                n.famountmt AS fnetnota,
                CAST(n.fsalesman AS text) AS fsalesman,
                m.fadminbank,
                m.fadjustment,
                dt.fqty
            ");

        $this->applyFilters($query, $filters, 'n.fsalesman');
