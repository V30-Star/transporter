...
class ListingHutangDagangController extends Controller
{
    ...
    private function getRawData(Request $request)
    {
        ...
        $base = DB::table('trstockmt as h')
            ...
            ->whereIn('h.fstockmtcode', ['BUY', 'RUB']);
        ...
        return DB::query()
            ...
            ->selectRaw("a.*, COALESCE(b.ftotalbayar, 0) AS ftotalbayar, COALESCE(c.ftotalsju, 0) AS ftotalsju, CASE WHEN a.fstockmtcode = 'RUB' THEN (a.fnilainota - (COALESCE(b.ftotalbayar, 0) + COALESCE(c.ftotalsju, 0))) * -1 ELSE a.fnilainota - (COALESCE(b.ftotalbayar, 0) + COALESCE(c.ftotalsju, 0)) END AS famountremain")
