...
class PelunasanCustomerController extends Controller
{
    ...
    public function pickableNota(Request $request)
    {
        ...
        $data = $query
            ...
            ->map(function ($row) {
                ...
                if (in_array(strtoupper($trCode), ['REJ', 'RUJ'], true)) {
                    $amount *= -1;
                    $amountSo *= -1;
                    if ($amountRemain < 0) {
                        $amountRemain *= -1;
    ...
    private function normalizeDetails(array $details): Collection
    {
        return collect($details)
            ->map(function (array $detail) {
                ...
                $isRetur = in_array($trCode, ['REJ', 'RUJ'], true);
                if ($isRetur && $rawKasdtValue >= 0) {
                ...
                $discPersen = $isRetur ? 0 : round((float) ($detail['fdiscpersen'] ?? 0), 2);
    ...
    private function buildJournalDetailEntries(Collection $detailRows, string $paymentDate, Customer $customer): Collection
    {
        ...
        return $detailRows->values()->map(function (array $row) use ($customer, $receivableAccount, $returnReceivableAccount, $referenceMap, $receivableCode, $returnReceivableCode) {
            ...
            $journalAmount = $isRetur
                ? $paymentAmount
                : round($paymentAmount + $discountAmount, 2);
            $faccountCode = $isRetur ? $returnReceivableCode : $receivableCode;
    ...
    private function resolveReferenceTransactions(Collection $detailRows, Carbon $paymentDate): array
    {
        ...
        $returStockDateMap = $refNos
            ->filter(fn($refNo) => preg_match('/^(REJ|RUJ)\./i', (string) $refNo))
            ->mapWithKeys(function ($refNo) {
                $trimmed = trim((string) $refNo);
                $rebNo = preg_replace('/^(REJ|RUJ)\./i', 'REB.', $trimmed);
        ...
        foreach ($detailRows as $index => $row) {
            ...
            $isReturDoc = in_array($trCode, ['REJ', 'RUJ'], true);
            $referenceLabel = $isReturDoc ? 'Tanggal retur' : 'Tanggal faktur';
            ...
            ? $header->details
                ...
                ->map(function ($detail, $index) use ($referenceRemainMap) {
                    ...
                    if ($reference) {
                        ...
                        $refTrCode = strtoupper(trim((string) ($reference->ftrcode ?? $trCode)));
                        $isRefRetur = in_array($refTrCode, ['REJ', 'RUJ'], true);
                    ...
                    $isTrRetur = in_array(strtoupper($trCode), ['REJ', 'RUJ'], true);
                    if ($isTrRetur) {
    ...
    public function print(string $fkasmtno)
    {
        ...
        $details = $details->map(function ($row) use ($invoices) {
            ...
            $invDate = $inv?->fsodate ?: $row->fdatetime;
