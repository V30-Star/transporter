...
<div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <form action="{{ route('rekening.update', $rekening->frekeningid) }}" method="POST" id="formRekening">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                <div class="px-4 pt-3 pb-0">
                    <p class="text-xs font-medium uppercase tracking-wide text-gray-400">Identitas Rekening</p>
                </div>
                <div class="p-4 space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="frekeningname" id="frekeningname" rows="3"
                            ...
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-blue-100 @error('frekeningname') border-red-400 @enderror {{ !empty($isTransactionLocked) ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'focus:border-blue-500' }}"
                            placeholder="cth. BCA CABANG JAKARTA - 1234567890" autofocus>{{ old('frekeningname', $rekening->frekeningname) }}</textarea>
                        @error('frekeningname')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                        @if(!empty($isTransactionLocked))
                            <p class="text-amber-600 text-xs mt-1 font-medium">Nama rekening tidak bisa diubah karena rekening sudah dipakai transaksi.</p>
                        @endif
                    </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                ...
                <div class="p-4 space-y-4">
                    ...
                    {{-- Status Aktif --}}
                    <div x-data="{ active: {{ old('fnonactive', $rekening->fnonactive) == '1' ? 'false' : 'true' }} }">
