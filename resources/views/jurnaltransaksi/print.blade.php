...
<head>
    <meta charset="utf-8">
    <title>Jurnal Transaksi - {{ $hdr->fjurnalno ?? '-' }}</title>
    ...
    <div id="raw-templates" style="display: none;">
        ...
        <div id="tpl-header">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">JURNAL TRANSAKSI</div>
                    <div class="so-no">No. {{ $hdr->fjurnalno ?? '-' }}</div>
                </div>
            </div>

            <div class="customer-container">
                <span class="customer-label">Informasi Jurnal</span>
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 10px;">
                    <div style="flex: 1; padding-right: 10px; border-right: 1px solid #000;">
                        <div style="font-size: 10px;">
                            <strong>Cabang:</strong> {{ $hdr->cabang_name ?? ($hdr->fbranchcode ?? '-') }}
                        </div>
                        @if(!empty($hdr->fjurnalnote))
                            <div style="font-size: 9px; margin-top: 3px; color: #333;">
                                <strong>Keterangan:</strong> {{ $hdr->fjurnalnote }}
                            </div>
                        @endif
                    </div>
        ...
        <table id="tpl-table">
            <thead id="tpl-thead">
                <tr>
                    ...
                    <th class="text-right" style="width: 80px;">Jumlah (Rp)</th>
                </tr>
            </thead>
            <tbody id="raw-rows">
                @foreach ($dt as $i => $r)
                    @php
                        ...
                        $subText = !empty($r->subaccount_name) ? ' [ ' . $r->subaccount_name . ' ]' : (!empty($r->fsubaccount) ? ' [ ' . $r->fsubaccount . ' ]' : '');
                    @endphp
                    {{-- Row 1: No & Account + SubAccount --}}
                    <tr class="item-row item-row-main">
                        <td class="text-center row-no">{{ $i + 1 }}</td>
                        <td colspan="3" style="font-weight: bold;">
                            {{ $r->faccount ?? '-' }} - {{ $r->account_name ?? '-' }}
                            @if(!empty($subText))
                                <span style="font-weight: normal; font-size: 9px; color: #444;">{{ $subText }}</span>
                            @endif
                        </td>
                    </tr>
                    {{-- Row 2: Note, D/K, Jumlah --}}
                    <tr class="item-row item-row-sub">
                        <td></td>
                        <td style="font-size: 9px; color: #333;">{{ $r->faccountnote ?? '-' }}</td>
            ...
            <div class="summary-container">
                ...
                <div style="width: 36%; padding: 0 6px;">
                    <div style="font-size: 9px; color: #333; font-style: italic;">
                        Terbilang:<br>
                        <strong># {{ strtoupper(terbilang($totalDebit)) }} RUPIAH #</strong>
                    </div>
                </div>
