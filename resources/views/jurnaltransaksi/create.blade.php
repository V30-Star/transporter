...
@section('content')
    ...
    <div>
            <form action="{{ route('jurnaltransaksi.store') }}" method="POST" data-form-draft="true"
                ...
                <div class="border border-gray-200 rounded-xl bg-white p-6 mb-6">
                    ...
                    <div class="lg:col-span-4" x-data="{ autoCode: true }">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="text" name="fjurnalno" class="w-full border-gray-300 rounded-lg px-3 py-2 uppercase focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div class="lg:col-span-2">
                        ...
                        @php
                            $currentType = old('fjurnaltype', ($fixedJournalType ?? ($journalType ?: 'SJU')));
                ...
                {{-- ── DETAIL jurnaldt ── --}}
                <div class="border border-gray-200 rounded-xl bg-white p-6 mb-6">
                    <div class="flex items-center justify-between gap-2 px-4 pt-3 pb-0 mb-4">
                                        ...
                                        <td class="p-2">
                                            <div class="flex">
                                                <input type="text" x-model="row.frefno"
                                                    ...
                                                    :readonly="!!referenceSource(row.faccount)"
                                                    :disabled="!isRefAllowed(row.faccount)"
                                                    :class="!isRefAllowed(row.faccount) || referenceSource(row.faccount) ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : 'bg-white'"
                                                    @keydown.prevent="notifyReferenceBrowseRequired(row)"
                                                    placeholder="No Ref">
                                                <button type="button" @click="openReferenceBrowse(index)"
                                                    ...
                                                    :disabled="!referenceSource(row.faccount)"
                                                    :class="!referenceSource(row.faccount) ? 'opacity-50 cursor-not-allowed' : ''"
    ...
    <script>
        ...
        function itemsTable() {
            return {
                ...
                notifyReferenceBrowseRequired(row) {
                    if (!this.referenceSource(row?.faccount)) return;
                ...
                openReferenceBrowse(index) {
                    ...
                    const source = this.referenceSource(row?.faccount);
                    if (!source) return;
                ...
                pickReference(ref) {
                    if (this.referenceRowIndex === null || !this.savedItems[this.referenceRowIndex]) return;
                    this.savedItems[this.referenceRowIndex].frefno = ref.ref_no || '';
