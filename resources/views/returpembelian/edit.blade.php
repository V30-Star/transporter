        ...
        showNoItems: false
                                ...
                                <div x-data="{ autoCode: true }">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <input type="text" name="fpono"
                                            ...
                                            value="{{ strtoupper(old('fpono') ?? ($displayFstockmtno ?? $returpembelian->fstockmtno ?? '')) }}"
                                            :disabled="autoCode"
                                            :required="!autoCode"
            ...
            @else
                ...
                <div class="space-y-4">
                        ...
                        @submit.prevent="
                            ...
                            if (duplicateCode) {
                                Swal.fire({
                                    ...
                                    text: `Kode produk ${duplicateCode} dengan nomor acak yang sama tidak boleh dobel dalam satu Retur Pembelian.`,
                                                ...
                                                <td class="p-2">
                                                    <div class="flex w-full max-w-full">
                                                        ...
                                                        <button type="button" @click="openDesc('edit')"
                                                            class="shrink-0 inline-flex items-center border border-l-0 rounded-r px-2 py-1 transition-colors"
                                                            :class="editRow.fdesc ? 'btn-desc-filled font-medium' : 'btn-desc-empty'"
                                                            title="Deskripsi item">
                                                            <x-heroicon-o-document-text class="w-4 h-4" />
                                                        </button>
                                                    </div>
                                                ...
                                                <td class="p-2">
                                                    <template x-if="editRow.units.length > 1">
                                                        <select class="w-full border rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500" x-ref="editUnit"
                                                            ...
                                                            <template x-for="(u, uIdx) in editRow.units" :key="uIdx">
                                                    ...
                                                    <td class="p-2">
                                                        <div class="flex w-full max-w-full">
                                                            ...
                                                            <button type="button" @click="openDesc('draft', di)"
                                                                class="shrink-0 inline-flex items-center border border-l-0 rounded-r px-2 py-1 transition-colors"
                                                                :class="dr.fdesc ? 'btn-desc-filled font-medium' : 'btn-desc-empty'"
                                                                title="Deskripsi item">
                                                                <x-heroicon-o-document-text class="w-4 h-4" />
                                                            </button>
                                                        </div>
                    ...
                    {{-- FOOTER INFO --}}
                    @php
                        $lastUpdate = $returpembelian->fupdatedat ?: $returpembelian->fcreatedat;
                        $updatedBy = $returpembelian->fuserupdate ?: ($returpembelian->fusercreate ?: '—');
                    @endphp
    ...
    @if ($action === 'delete' && $canDeletePermission)
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 allow-action">
                <h3 class="text-lg font-semibold mb-4">Hapus Retur Pembelian ini?</h3>
                <form id="deleteForm" action="{{ route('returpembelian.destroy', $returpembelian->fstockmtid) }}"
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <button type="button" id="btnYa" onclick="confirmDelete()" @if ($usageLocked) disabled @endif
...
<script>
    ...
    window.getReturPembelianDuplicateCode = function(form) {
        ...
        for (const input of inputs) {
            ...
            const ref = form.querySelectorAll('input[name="frefdtno[]"]')[[...inputs].indexOf(input)]?.value?.trim() || '';
            const key = `${code}|${ref}`;
            if (seen.has(key)) {
                return code;
            }
            ...
            addManyFromPR(header, items) {
                ...
                items.forEach(src => {
                    const row = {
                        ...
                        fprice: (src.fpricenet !== undefined && src.fpricenet !== null && parseFloat(src.fpricenet) > 0) ? Math.max(0, parseFloat(src.fpricenet) - parseFloat(src.fbiaya || 0)) : Number(src.fprice ?? 0),
                        fpriceInput: this.fmt((src.fpricenet !== undefined && src.fpricenet !== null && parseFloat(src.fpricenet) > 0) ? Math.max(0, parseFloat(src.fpricenet) - parseFloat(src.fbiaya || 0)) : Number(src.fprice ?? 0)),
                ...
                initDataTable() {
                    ...
                    this.table = $('#productTable').DataTable({
                        ...
                        ajax: {
                            ...
                            type: 'GET',
                            data: (d) => {
                                const ftypebuy = @json((int) $currentType);
                                const result = {
                                    draw: d.draw,
                                    start: d.start,
                                    length: d.length,
