...
<head>
    ...
    <title>Laporan Assembling</title>
    <!-- Fonts -->
    ...
    <style>
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 32mm 18mm 28mm 1fr 14mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 22mm 1fr 18mm 18mm 22mm 24mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Assembling</div>
                </div>
            </div>
        ...
        <div class="po-detail-labels">
            ...
            <div style="text-align: right;">Qty.Rijeks</div>
            <div style="text-align: right;">@ HPP</div>
            <div style="text-align: right;">Total HPP</div>
        </div>

        @foreach ($prhData as $penerimaanbarang)
            @php
                $viewUrl = route('assembling.view', $penerimaanbarang->fstockmtid);
                $editUrl = route('assembling.edit', $penerimaanbarang->fstockmtid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div class="truncate" title="{{ $penerimaanbarang->fstockmtno }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $penerimaanbarang->fstockmtno }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $penerimaanbarang->fstockmtno }}</span>
                    </div>
                    <div>{{ \Carbon\Carbon::parse($penerimaanbarang->fstockmtdate)->format('d-m-Y') }}</div>
                    <div class="truncate">{{ $penerimaanbarang->fwhname }}</div>
                    <div class="truncate" title="{{ $penerimaanbarang->fket ?? 'LOCO BL' }}">
                        {{ $penerimaanbarang->fket ?? 'LOCO BL' }}
                    </div>
                    <div class="truncate">{{ $penerimaanbarang->fusercreate }}</div>
                </div>
    ...
    <div class="report-wrapper" id="reportWrapper">
        @if (empty($prhData) || count($prhData) === 0)
