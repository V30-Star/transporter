...
@section('content')
    ...
    <div x-data="{ subAccount: {{ old('fhavesubaccount', $account->fhavesubaccount) ? 'true' : 'false' }} }">
            ...
            <form action="{{ route('account.update', $account->faccid) }}" method="POST" id="formAccount">
                ...
                <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div>
                            ...
                            <div class="flex">
                                <input type="text" id="headerDisplay"
                                    ...
                                    placeholder="Pilih account header..." readonly
                                    @if (empty($isUsedInTransaction)) @click="window.dispatchEvent(new CustomEvent('account-browse-open'))" @endif
                                    value="{{ old('faccupline', $account->faccupline) ? ($selectedHeader ? $selectedHeader->faccount . ' — ' . $selectedHeader->faccname : old('faccupline')) : '' }}">
                                <button type="button"
                                    @if (empty($isUsedInTransaction)) @click="window.dispatchEvent(new CustomEvent('account-browse-open'))"
                                ...
                                @else
                                disabled
                                class="border border-gray-300 rounded-r-lg px-3 py-2 bg-gray-50 text-gray-300 cursor-not-allowed" @endif
                        ...
                        <div id="selectedHeaderBadge" class="{{ old('faccupline', $account->faccupline) ? 'inline-flex' : 'hidden' }} mt-2 items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700 font-medium">
                            ...
                            @enderror
                            @if (!empty($isUsedInTransaction))
                                <p class="text-amber-600 text-xs mt-1 font-medium">Account header tidak bisa diubah karena
                ...
                <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                    ...
                    <div class="p-4 space-y-4">
                        ...
                        {{-- Sub Account Toggle --}}
                        <div>
                            <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 transition-colors"
                                :class="{{ $isUsedInTransaction ? 'true' : 'false' }} ? 'cursor-not-allowed opacity-60' :
                                ...
                                @if (!empty($isUsedInTransaction))
                                    <div
                                        class="relative w-10 h-6 rounded-full flex-shrink-0 bg-gray-200 border border-gray-300">
                                        <div class="absolute w-4 h-4 bg-gray-400 rounded-full top-1 transition-transform duration-200 shadow-sm"
                                            :class="subAccount ? 'translate-x-5 left-1' : 'left-1'"></div>
                                    </div>
                                @else
                                    <div class="relative w-10 h-6 rounded-full transition-colors duration-200 flex-shrink-0"
                                        :class="subAccount ? 'bg-indigo-600' : 'bg-gray-300'">
                                        <div class="absolute w-4 h-4 bg-white rounded-full top-1 transition-transform duration-200 shadow-md"
                                            :class="subAccount ? 'translate-x-5 left-1' : 'left-1'"></div>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div x-show="subAccount" x-transition class="mt-3 pl-1">
                                ...
                                <div class="flex gap-2 flex-wrap" x-data="{ sub: '{{ old('ftypesubaccount', $subType) }}' }">
                                    ...
                                    @foreach (['Sub Account', 'Customer', 'Supplier'] as $opt)
                                        <button type="button" @click="sub='{{ $opt }}'" {{-- Kunci tombol di html jika sedang digunakan dalam transaksi --}}
                                            @if (!empty($isUsedInTransaction)) disabled @endif
                                            :class="sub === '{{ $opt }}' ?
                                                {{-- Style ketika terpilih --}}('{{ !empty($isUsedInTransaction) }}' ?
                                                    'bg-gray-400 border-gray-400 text-white cursor-not-allowed' :
                                                    'bg-indigo-600 border-indigo-600 text-white font-semibold shadow-sm'
                                                ) :
                                                {{-- Style ketika tidak terpilih --}}('{{ !empty($isUsedInTransaction) }}' ?
                                                    'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' :
                                                    'bg-white border-gray-300 text-gray-600 hover:border-gray-400 hover:bg-gray-50'
                                                )"
                                            class="px-4 py-1.5 rounded-full text-xs border transition-all focus:outline-none">
                                            {{ $opt }}
                                        </button>
                                    @endforeach
                                </div>
        ...
        <div x-data="accountBrowser()" x-show="open" x-cloak x-transition.opacity
            ...
            <div class="relative bg-white rounded-2xl w-full max-w-3xl flex flex-col overflow-hidden"
                style="height:580px">
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4" style="min-height:0">
                    <table id="accountTable" class="min-w-full text-sm display" style="width:100%">
                        <thead>
                            <tr class="bg-gray-50">
                                <th class="text-left p-3 text-xs font-medium text-gray-500 border-b border-gray-200 w-1/3">
                                    Kode Account</th>
                                <th class="text-left p-3 text-xs font-medium text-gray-500 border-b border-gray-200">Nama
                                    Account</th>
                                <th
                                    class="text-center p-3 text-xs font-medium text-gray-500 border-b border-gray-200 w-24">
                                    Pilih</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    ...
                    this.table = $('#accountTable').DataTable({
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                width: '18%',
                                render: () =>
                                    `<button type="button" class="btn-choose px-3 py-1 rounded-md text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white transition">Pilih</button>`
                            }
                        ],
                        pageLength: 10,
