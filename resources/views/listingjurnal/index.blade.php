                ...
                <form method="GET" action="{{ route('listingjurnal.print') }}" target="_blank">
                    <div class="space-y-4">
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Cabang</label>
                                @if ($isAuthorized)
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Jurnal
                                    Type</label>
                                <div class="flex space-x-2">
                                    ...
                                    @foreach ($typeOptions as $type)
                                        <label class="flex items-center text-sm cursor-pointer select-none">
                                            <input type="checkbox" name="journal_types[]" value="{{ $type->fmastercode }}"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500 w-4 h-4"
                                                checked>
                                            <span class="text-gray-700 font-medium">{{ $type->fmastercode }} -
                                                {{ $type->fmastername }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        {{-- Urut Berdasarkan --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Urut
                                Berdasarkan</label>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t mt-6">
                        <button type="button" onclick="window.location.href='{{ route('dashboard') }}'"
                            class="px-5 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200">Cancel</button>
                        <button type="submit"
