...
<head>
    ...
    <style>
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 16mm 38mm 18mm 1fr 14mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 22mm 45mm 1fr 24mm 22mm 22mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    ...
    <div class="no-print">
        ...
        <a href="{{ route('listingjurnal.excel', request()->query()) }}"
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Jurnal Transaksi</div>
                </div>
            </div>
        ...
        @foreach ($groupedData as $jurnalNo => $lines)
            @php
                ...
                $type = strtoupper(trim((string) ($firstLine->fjurnaltype ?? '')));
                if ($type === 'BKM') {
                    $viewUrl = route('penerimaankas.view', $jurnalNo);
                    $editUrl = route('penerimaankas.edit', $jurnalNo);
                } elseif ($type === 'BKK') {
                    $viewUrl = route('pengeluarankas.view', $jurnalNo);
                    $editUrl = route('pengeluarankas.edit', $jurnalNo);
                } elseif ($type === 'RCP') {
                    $viewUrl = route('pelunasancustomer.view', $jurnalNo);
                    $editUrl = route('pelunasancustomer.edit', $jurnalNo);
                } elseif ($type === 'PAY') {
            ...
            <div class="journal-block">
                <div class="po-header">
                    ...
                    <div class="truncate">{{ $firstLine->fuserid }}</div>
                </div>
                ...
                @foreach ($lines as $dt)
                    ...
                    @endphp
                    <div class="po-detail">
                        <div class="truncate">{{ $dt->faccount }}</div>
        ...
        <div class="po-totals-panel-wrapper">
            ...
            <div class="po-totals-container">
                <div class="po-total-row">
                    ...
                    <span>Rp {{ number_format($grandTotalDebet, 2, ',', '.') }}</span>
                </div>
                <div class="po-total-row grand-total-row">
                    <span>TOTAL KREDIT</span>
                    <span>Rp {{ number_format($grandTotalKredit, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function openTrxActionModal(event, sono, viewUrl, editUrl, canEdit) {
        ...
        const btnEdit = document.getElementById('btnEditTrx');
        if (canEdit && editUrl) {
