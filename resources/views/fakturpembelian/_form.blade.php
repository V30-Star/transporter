@php
    ...
    $formMode = $formMode ?? ($isDeleteMode ? 'delete' : ($isReadOnly ? 'view' : 'edit'));
    $readonlyTextClass = $isReadOnly ? ' text-gray-700' : '';
    $currentAccountCode = trim((string) old('fprdjadi', $currentAccount ?? ''));
    $currentAccountName = trim((string) ($currentAccountName ?? ''));
    $useBiayaModelBinding = $useBiayaModelBinding ?? false;
    $biayaInputId = $biayaInputId ?? ($useBiayaModelBinding ? null : 'editBiayaGlobalInput');
    $biayaButtonAction = $biayaButtonAction ?? ($useBiayaModelBinding ? 'alokasiBiaya()' : 'window.applyFpbBiayaGlobalFromHeader?.()');
    $biayaInputValue = $biayaInputValue ?? number_format((float) ($biayaGlobal ?? 0), 2, '.', '');
@endphp
...
@if ($routeName === 'fakturpembelian.create')
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
    ...
    <div class="lg:col-span-4" x-data="{ autoCode: true }">
        ...
        <div class="flex items-center gap-3">
            <input type="text" name="fstockmtno" class="w-full border rounded px-3 py-2{{ $readonlyTextClass }}"
                value="{{ old('fstockmtno', $currentDocNo ?? '') }}" :disabled="autoCode"
                :class="autoCode ? 'bg-gray-200 cursor-not-allowed' : 'bg-white'">
            <label class="inline-flex items-center select-none">
                <input type="checkbox" x-model="autoCode" checked {{ $isReadOnly ? 'disabled' : '' }}>
    ...
    <div class="lg:col-span-4">
        ...
        <select name="ftypebuy" x-model="selectedType"
            class="w-full border rounded px-3 py-2{{ $readonlyTextClass }} @error('ftypebuy') border-red-500 @enderror"
            {{ $isReadOnly ? 'disabled' : '' }}>
            <option value="0" {{ (old('ftypebuy', $currentType ?? '0') == '0') ? 'selected' : '' }}>{{ 'Stok' }}</option>
            <option value="1" {{ (old('ftypebuy', $currentType ?? '0') == '1') ? 'selected' : '' }}>{{ 'Non Stok' }}</option>
            <option value="2" {{ (old('ftypebuy', $currentType ?? '0') == '2') ? 'selected' : '' }}>{{ 'Uang Muka' }}</option>
        </select>
        @error('ftypebuy')
    ...
    <div class="lg:col-span-4">
        ...
        <input type="date" id="fstockmtdate" name="fstockmtdate"
            value="{{ old('fstockmtdate', $currentDate ?? date('Y-m-d')) }}"
            class="w-full border rounded px-3 py-2{{ $readonlyTextClass }} @error('fstockmtdate') border-red-500 @enderror"
            {{ $isReadOnly ? 'disabled' : '' }}>
        @error('fstockmtdate')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
        ...
        <div class="flex">
            <div class="relative flex-1" for="modal_filter_supplier_id">
                ...
                <div id="supplierBrowseOverlay" class="absolute inset-0" role="button" aria-label="Browse supplier"
                    ...
                    {{ $isReadOnly ? 'hidden' : '' }}></div>
            </div>
            <input type="hidden" name="fsupplier" id="supplierCodeHidden"
                value="{{ old('fsupplier', $currentSupplier ?? '') }}">
            <button type="button" id="supplierBrowseButton"
                @click="window.dispatchEvent(new CustomEvent('supplier-browse-open'))"
                class="border -ml-px px-3 py-2 bg-white hover:bg-gray-50 rounded-r-none{{ $isReadOnly ? ' opacity-50 cursor-not-allowed' : '' }}"
                title="Browse Supplier" {{ $isReadOnly ? 'disabled' : '' }}>
                <x-heroicon-o-magnifying-glass class="w-5 h-5" />
            </button>
            <a href="{{ route('supplier.create') }}" target="_blank" rel="noopener" id="supplierCreateButton"
                class="border -ml-px rounded-r px-3 py-2 bg-white hover:bg-gray-50{{ $isReadOnly ? ' pointer-events-none opacity-50' : '' }}"
    ...
    <div class="lg:col-span-4">
        ...
        <div class="flex">
            <div class="relative flex-1">
                <select id="warehouseSelect"
                    ...
                    @foreach ($warehouses as $wh)
                        <option value="{{ $wh->fwhcode }}" data-id="{{ $wh->fwhid }}" data-branch="{{ $wh->fbranchcode }}"
                            {{ old('ffrom', $currentWarehouse ?? '') == $wh->fwhcode ? 'selected' : '' }}>
            ...
            <input type="hidden" name="ffrom" id="warehouseCodeHidden" value="{{ old('ffrom', $currentWarehouse ?? '') }}">
            <button type="button"
                @click="window.dispatchEvent(new CustomEvent('faktur-pembelian-warehouse-browse-open'))"
                class="border -ml-px px-3 py-2 bg-white hover:bg-gray-50 rounded-r-none{{ $isReadOnly ? ' opacity-50 cursor-not-allowed' : '' }}"
                title="Browse Gudang" {{ $isReadOnly ? 'disabled' : '' }}>
                <x-heroicon-o-magnifying-glass class="w-5 h-5" />
            </button>
        ...
        <div class="flex">
            <div class="relative flex-1">
                <select id="accountSelect" class="w-full border rounded-l px-3 py-2"
                    ...
                    <option value=""></option>
                    @if ($currentAccountCode !== '' && !$accounts->contains('faccount', $currentAccountCode))
                        <option value="{{ $currentAccountCode }}" data-faccid="{{ old('faccid', $currentAccountId ?? '') }}"
                            selected>
                            {{ $currentAccountCode }}{{ $currentAccountName !== '' ? ' - '.$currentAccountName : '' }}
                        </option>
                    @endif
                    @foreach ($accounts as $account)
                        <option value="{{ $account->faccount }}" data-faccid="{{ $account->faccid }}"
                            data-branch="{{ $account->faccount }}"
                            {{ $currentAccountCode == $account->faccount ? 'selected' : '' }}>
                ...
                <div class="absolute inset-0" role="button" aria-label="Browse account"
                    ...
                    x-show="selectedType == '1'" {{ $isReadOnly ? 'hidden' : '' }}></div>
            </div>

            <input type="hidden" name="fprdjadi" id="accountCodeHidden" value="{{ old('fprdjadi', $currentAccount ?? '') }}">
            <input type="hidden" name="faccid" id="accountIdHidden" value="{{ old('faccid', $currentAccountId ?? '') }}">

            <button type="button"
                @click="window.dispatchEvent(new CustomEvent('account-browse-open'))"
                class="border -ml-px px-3 py-2 bg-white hover:bg-gray-50 rounded-r-none"
                :disabled="selectedType != '1'" :class="{ 'opacity-50 cursor-not-allowed': selectedType != '1' }"
                {{ $isReadOnly ? 'disabled' : '' }}
    ...
    <div class="lg:col-span-4">
        ...
        <div class="flex items-center gap-3">
            <input type="text" name="frefno" class="w-full border rounded px-3 py-2{{ $readonlyTextClass }}"
                value="{{ old('frefno', $currentRefNo ?? '') }}" {{ $isReadOnly ? 'disabled' : '' }}>
            <label class="inline-flex items-center select-none"></label>
        </div>
    </div>

    <div class="lg:col-span-2">
        <label class="block text-sm font-medium">TOP (Hari)</label>
        <input type="number" id="ftempohr" name="ftempohr" value="{{ old('ftempohr', $currentTempo ?? '0') }}"
            class="w-full border rounded px-3 py-2{{ $readonlyTextClass }} @error('ftempohr') border-red-500 @enderror"
            {{ $isReadOnly ? 'disabled' : '' }}
    ...
    <div class="lg:col-span-2">
        ...
        <input type="date" id="fjatuhtempo" name="fjatuhtempo" value="{{ old('fjatuhtempo', '') }}" readonly
            class="w-full border rounded px-3 py-2 bg-gray-100{{ $readonlyTextClass }} @error('fjatuhtempo') border-red-500 @enderror">
        @error('fjatuhtempo')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
...
<div class="fpb-ket-biaya mt-4">
    <div class="min-w-0">
        ...
        <textarea name="fket" rows="3" class="w-full border rounded px-3 py-2{{ $readonlyTextClass }} @error('fket') border-red-500 @enderror"
            {{ $isReadOnly ? 'disabled' : '' }}
            placeholder="Tulis keterangan tambahan di sini...">{{ old('fket', $currentNote ?? '') }}</textarea>
        @error('fket')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="min-w-0 overflow-hidden">
        ...
        <div class="hpp-box h-full min-h-[96px] bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm flex items-center gap-3">
            <input type="{{ $isReadOnly ? 'text' : 'number' }}"
                @if ($biayaInputId) id="{{ $biayaInputId }}" @endif
                value="{{ $biayaInputValue }}" placeholder="Masukkan Total Ongkir"
                @if ($useBiayaModelBinding && !$isReadOnly) x-model.number="biayaGlobal" @endif
                class="flex-1 border rounded px-3 py-2 text-right font-mono bg-white{{ $isReadOnly ? ' text-gray-700' : '' }}"
                {{ $isReadOnly ? 'disabled' : '' }}>

            <button type="button"
                @if (!$isReadOnly && $useBiayaModelBinding)
                    @click="{{ $biayaButtonAction }}"
                @else
                    onclick="{{ $biayaButtonAction }}"
                @endif
                class="shrink-0 min-w-[120px] bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded transition flex items-center justify-center gap-2{{ $isReadOnly ? ' opacity-50 cursor-not-allowed' : '' }}"
                {{ $isReadOnly ? 'disabled' : '' }}>
                Hitung
            </button>
        </div>
                            ...
                            <table class="min-w-full text-sm balanced-detail-table fpb-detail-table" data-skip-auto-detail-style="true">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-t align-top">
                                        ...
                                        <td class="p-2">
                                            <div class="flex w-full max-w-full">
                                                <input type="text" class="min-w-0 flex-1 border rounded-l px-2 py-1 font-mono"
...
@if (in_array($routeName, ['fakturpembelian.edit', 'fakturpembelian.delete'], true))
                ...
                Hitung
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            const fstockmtdate = document.getElementById('fstockmtdate');
                            if (fstockmtdate) {
                                fstockmtdate.addEventListener('change', calculateDueDate);
                            }

                            const ftempohr = document.getElementById('ftempohr');
                            if (ftempohr) {
                        ...
                        <div id="itemsTableRoot" x-data="itemsTable()" x-init="init()" class="mt-6 space-y-2">
                            ...
                            <div x-show="showSupplierRequired" x-cloak
                                ...
                                <div class="relative bg-white w-[92vw] max-w-md rounded-2xl shadow-2xl overflow-hidden"
                                    ...
                                    <div class="px-5 py-4">
                                        <p class="text-sm text-gray-700">
                                            Supplier wajib dipilih sebelum input produk manual. Untuk Add PO atau Add
                                            TER, supplier tidak wajib dipilih terlebih dahulu.
                                        </p>
                                    </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (sel) {
                    let opt = [...sel.options].find(o => o.value == currentCode);
                    const label = `${currentCode} - ${currentName}`;

                    if (!opt) {
                        opt = new Option(label, currentCode, true, true);
                        sel.add(opt);
                    } else {
                        opt.text = label;
                        opt.selected = true;
...
@if ($routeName === 'fakturpembelian.view')

    @php
        $permissions = explode(',', session('user_restricted_permissions', ''));
        $canPrint = in_array('printFakturPembelian', $permissions, true) || in_array('viewTr_prh', $permissions, true) || in_array('updateFakturPembelian', $permissions, true) || in_array('deleteFakturPembelian', $permissions, true) || in_array('createFakturPembelian', $permissions, true);
    @endphp
    <style>
        input:focus,
        select:focus,
                            ...
                            <tbody>
                                ...
                                <tr x-show="editingIndex !== null" class="border-t align-top" x-cloak>
                                    ...
                                    <td class="p-2">
                                        <input type="text"
                                            class="w-full border rounded px-2 py-1 bg-gray-100 text-gray-600 text-sm text-right"
                                            :value="fmt(editRow.ftotprice)" disabled>
                                    </td>
