...
@section('title', $action === 'delete' ? 'Faktur Pembelian - Delete' : ($action === 'view' ? 'Faktur Pembelian - View' :
...
@section('content')
    @php
        ...
        $canPrintPermission =
            in_array('printFakturPembelian', $permissions, true) ||
            in_array('viewTr_prh', $permissions, true) ||
            in_array('updateFakturPembelian', $permissions, true) ||
            in_array('deleteFakturPembelian', $permissions, true) ||
            in_array('createFakturPembelian', $permissions, true);
    ...
    @php
        ...
        $currentPpnAmount = old(
            'famountpajak',
            $ppnAmount ?? ($fakturpembelian->famountpopajak ?? ($fakturpembelian->famountpajak ?? 0)),
        );
    ...
    <div x-data="itemsTable()" x-init="init()" class="lg:col-span-5">
                    @if ($isReadOnly)
                {{-- Read Only View Mode --}}
                <div class="space-y-4">
                    {{-- ─── CARD 1: Identitas Faktur Pembelian (ReadOnly) ────────────────────── --}}
                        ...
                        <div class="p-4 space-y-3">
                            <div class="grid grid-cols-3 gap-3">
                                ...
                                <div x-data="{ autoCode: true }">
                                    <label class="block text-xs font-bold mb-1">
                                        No.Transaksi# <span class="text-red-500" x-show="!autoCode">*</span>
                                    </label>
                                    <div class="flex items-center gap-2">
                                        <input type="text" name="fstockmtno"
                                            ...
                                            value="{{ strtoupper(old('fstockmtno', $displayFstockmtno ?? $fakturpembelian->fstockmtno ?? '')) }}"
                                            :disabled="autoCode"
                                            :required="!autoCode"
                                ...
                                <div class="w-1/2">
                                    ...
                                    <select name="ftypebuy" disabled
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">
                                        <option value="0" {{ (string) old('ftypebuy', $fakturpembelian->ftypebuy) === '0' ? 'selected' : '' }}>Stok</option>
                                        <option value="1" {{ (string) old('ftypebuy', $fakturpembelian->ftypebuy) === '1' ? 'selected' : '' }}>Non Stok</option>
                                        <option value="2" {{ (string) old('ftypebuy', $fakturpembelian->ftypebuy) === '2' ? 'selected' : '' }}>Uang Muka</option>
                                    </select>
                                    <input type="hidden" name="ftypebuy" value="{{ old('ftypebuy', $fakturpembelian->ftypebuy) }}">
                                </div>
                                    ...
                                    <select name="fprdjadi" disabled
                                        ...
                                        @foreach ($accounts as $account)
                                            <option value="{{ $account->faccount }}"
                                                {{ old('fprdjadi', $fakturpembelian->fprdjadi) == $account->faccount ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="frefpo" value="{{ old('frefpo', $fakturpembelian->frefpo) }}" disabled
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">
                                </div>
                             ...
                             <div class="grid grid-cols-3 gap-3">
                                <div class="col-span-2">
                                    ...
                                    <textarea name="fket" rows="2" disabled readonly
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">{{ old('fket', $fakturpembelian->fket) }}</textarea>
                                </div>
                                            ...
                                            <tr class="border-t align-top hover:bg-gray-50">
                                                ...
                                                <td class="p-2 text-right">
                                                    <div class="px-2 py-1 text-sm text-gray-700 bg-gray-50 border rounded text-right font-medium"
                                                        x-text="formatTransactionAmount(it.fqty)"></div>
                                                </td>

                                                {{-- @ Harga --}}
                                                <td class="p-2 text-right">
                                                    <div class="px-2 py-1 text-sm text-slate-700 bg-slate-50 border rounded text-right font-medium"
                                                        x-text="formatTransactionAmount(it.fprice)"></div>
                                                </td>

                                                {{-- @ Biaya --}}
                                                <td class="p-2 text-right">
                                                    <div class="px-2 py-1 text-sm text-slate-700 bg-slate-50 border rounded text-right font-medium"
                                                        x-text="formatTransactionAmount(it.fbiaya)"></div>
                                                </td>

                                                {{-- Disc. % --}}
                                                <td class="p-2 text-right">
                                                    <div class="px-2 py-1 text-sm text-slate-700 bg-slate-50 border rounded text-right"
                                                        x-text="it.fdiscpersen"></div>
                                                </td>
                                ...
                                <div class="relative bg-white w-[92vw] max-w-lg rounded-2xl shadow-2xl overflow-hidden"
                                    ...
                                    <div class="px-5 py-4 space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <textarea x-model="descValue" rows="5" disabled class="w-full border rounded px-3 py-2 bg-gray-50 cursor-not-allowed text-sm text-gray-700"></textarea>
                                        </div>
                                    </div>
                        ...
                        <div class="flex items-center justify-end gap-3 px-5 py-2 bg-gray-50">
                            ...
                            <div class="flex gap-2">
                                ...
                                @elseif ($action === 'view' && $canPrintPermission)
                                    @php
                                        $isApproved = (int) ($fakturpembelian->fapproval ?? 0) === 1;
                                        $isPrinted = ! can_print_again() && (int) ($fakturpembelian->fprint ?? 0) === 1;
                                    @endphp
                                    @if (!$isApproved)
                                        <button type="button"
                                            onclick="Swal.fire({ icon: 'warning', title: 'Informasi', text: 'Faktur Pembelian belum di-approve dan tidak boleh dicetak.', confirmButtonColor: '#3b82f6' })"
            ...
            @else
                ...
                <div class="space-y-4">
                    <form action="{{ route('fakturpembelian.update', $fakturpembelian->fstockmtid) }}" method="POST"
                        data-form-draft="true" data-draft-key="fakturpembelian:edit:{{ $fakturpembelian->fstockmtid }}" x-data="itemsTable()" x-init="init()"
                        ...
                        {{-- ─── CARD 1: Identitas Faktur Pembelian (Edit) ────────────────────── --}}
                        <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                            <div class="flex items-center gap-2 px-4 pt-3 pb-0">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="p-4 space-y-3">
                                <div class="grid grid-cols-3 gap-3">
                                    ...
                                    <div class="w-1/2">
                                        ...
                                        <select name="ftypebuy" x-model="selectedType" @change="onTypeBuyChange()"
                                        ...
                                        <input type="hidden" name="ftypebuy" :value="selectedType">
                                        @error('ftypebuy')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                     ...
                                     <div>
                                        ...
                                        <div class="flex">
                                            ...
                                            <input type="hidden" name="fprdjadi" id="accountCodeHidden" value="{{ old('fprdjadi', $fakturpembelian->fprdjadi) }}">
                                            <input type="hidden" name="faccid" id="accountIdHidden" value="{{ old('faccid', $fakturpembelian->faccid) }}">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex">
                                            ...
                                            <input type="hidden" name="fsupplier" id="supplierCodeHidden" value="{{ old('fsupplier', $fakturpembelian->fsupplier) }}">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex">
                                            ...
                                            <input type="hidden" name="ffrom" id="warehouseCodeHidden" value="{{ old('ffrom', $fakturpembelian->ffrom) }}">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="frefpo" value="{{ old('frefpo', $fakturpembelian->frefpo) }}" :required="includePPN"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400" fill="none"
                            ...
                            <div class="p-4 space-y-3">
                                        ...
                                        <template x-for="(it, i) in savedItems" :key="it.uid">
                                            <tbody>
                                                <tr class="border-t border-gray-150 align-top hover:bg-gray-50">
                                                    ...
                                                    <td class="p-2" style="width: 20rem; min-width: 20rem;">
                                                        <div class="desc-inline-field flex w-full min-w-0 flex-nowrap items-stretch">
                                                            <div class="desc-inline-field__text min-w-0 flex-1 rounded-l-lg border border-gray-300 bg-gray-50 px-2 py-1 text-sm leading-5 text-gray-600 whitespace-normal break-words"
                                                                ...
                                                                x-text="it.fitemname"></div>
                                                            <button type="button" @click="openDesc('saved', i)"
                                                                class="shrink-0 inline-flex items-center border border-l-0 rounded-r-lg px-2 py-1 transition-colors"
                                                                :class="descButtonClass(it.fdesc)" title="Deskripsi">
                                                                <x-heroicon-o-document-text class="w-4 h-4" />
                                                            </button>
                                                        </div>
                                                    ...
                                                    <td class="p-2">
                                                        <div class="flex w-full max-w-full">
                                                            <div class="min-w-0 flex-1 rounded-l-lg border border-gray-300 bg-gray-50 px-2 py-1 text-sm leading-5 text-gray-600 whitespace-normal break-words"
                                                                x-text="it.frefdtno || '-'"></div>
                                                            <button type="button" @click="openProductHistory(it)"
                                                                class="shrink-0 inline-flex items-center border border-l-0 rounded-r-lg bg-slate-50 px-2 py-1 text-slate-700 hover:bg-slate-100 transition-colors border-slate-200"
                                                                :disabled="!canOpenHistory(it) || '{{ $action }}' === 'view'"
                                                                :class="(!canOpenHistory(it) || '{{ $action }}' === 'view') ? 'opacity-50 cursor-not-allowed' : ''"
                                ...
                                <div class="mt-3 flex justify-start items-start gap-4 flex-wrap">
                                    <div class="flex gap-2">
                                        {{-- Add PO button & modal --}}
        ...
        function itemsTable() {
            return {
                ...
                enforceQtyRow(row) {
                    ...
                    const refQty = Number(row.ref_qty || row.source_qty || row.po_qty || row.pb_qty || row.fqtysisa_source || row.maxqty || 0);
                ...
                onPoPicked(e) {
                    ...
                    this.addManyFromSource(header, pickItems, 'PO');
                },
                ...
                addManyFromSource(header, items, sourceType) {
                    ...
                    items.forEach(src => {
                        ...
                        const row = {
                            ...
                            fqtyremain_source: Number(src.fqtysisa ?? sourceQtyKecil ?? 0),
                            ref_qty: sourceLimit,
                ...
                isRowSavable(row) {
                    ...
                    return !!(code && satuan && (allowMinus ? qty !== 0 : qty > 0));
                },
                ...
                onTypeBuyChange() {
                    if (this.isUangMuka()) {
                        ...
                        if (hasInvalid) {
                            const message = "Tipe Faktur Uang Muka hanya boleh menggunakan produk kode UM. Item non-UM telah dihapus.";
                            if (typeof window.showAppWarningAlert === 'function') {
                                window.showAppWarningAlert('Informasi', message);
                            } else if (typeof Swal !== 'undefined') {
                                Swal.fire({ icon: 'warning', title: 'Informasi', text: message });
                ...
                init() {
                    ...
                    window.addEventListener('product-chosen', (e) => {
                        ...
                        if (this.browseTarget === 'saved' && this.browseIndex !== null) {
                            ...
                            const row = typeof this.browseIndex === 'number' ? this.savedItems[this.browseIndex] :
                                null;
                            if (row) {
                                apply(row);
                            }
                ...
                submitForm(form) {
                    ...
                    for (const row of validRows) {
                        ...
                        if (refQty > 0 && inputQty > refQty) {
                            const refDoc = String(row?.frefdtno || row?.frefpr || row?.fpono || '').trim();
                            const refLabel = refDoc.startsWith('RUB') ? 'RUB' : (refDoc.startsWith('UMB') ? 'UMB' : 'Referensi');
                            const message = `Qty item ${code} tidak boleh melebihi Qty ${refLabel} (${refQty}).`;
                            if (typeof window.showAppWarningAlert === 'function') {
                                window.showAppWarningAlert('WARNING', message);
                            } else if (typeof Swal !== 'undefined') {
                ...
                openBrowseFor(where, index = null) {
                    ...
                    const isNonStok = (document.querySelector('select[name="ftypebuy"]')?.value || this.selectedType || '').toString().trim() === '1';
                    window.dispatchEvent(new CustomEvent('browse-open', {
                        detail: {
                            forEdit: false,
                            codePrefix: isUM ? 'UM' : '',
                            ftypeFilter: isNonStok ? 'JASA' : ''
                        }
                    }));
                },
            };
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Hitung saat halaman dimuat (jika TOP sudah terisi)
        hitungJatuhTempo();
    });
</script>
