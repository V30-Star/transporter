...
@section('title', 'Laporan Penjualan DP')
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-4">
                <h3 class="text-xl font-bold text-gray-800">Laporan Penjualan DP</h3>
                <button onclick="toggleModal(false)" class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
            </div>

            <form id="reportForm" method="GET" action="{{ route('reportingpenjualandp.print') }}" target="_blank">
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            <label class="block text-xs font-bold uppercase mb-2">Filter Sisa DP</label>
                            <label class="block text-sm mb-1"><input type="radio" name="sisa_dp_filter" value="SEMUA_FAKTUR" checked> Semua Faktur</label>
                            <label class="block text-sm"><input type="radio" name="sisa_dp_filter" value="SISA_DP_GT_0"> Sisa DP &gt; 0</label>
                        </div>
                        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            <label class="block text-xs font-bold uppercase mb-2">Tipe Laporan</label>
                            <label class="block text-sm mb-1"><input type="radio" name="report_type" value="DETAIL" checked> Detail</label>
                            <label class="block text-sm"><input type="radio" name="report_type" value="REKAP"> Rekap</label>
                        </div>
                    </div>
                </div>

                <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
                    <button type="submit" onclick="setAction('{{ route('reportingpenjualandp.print') }}')" class="px-5 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">Preview</button>
                    <button type="submit" onclick="setAction('{{ route('reportingpenjualandp.print') }}')" class="px-5 py-2 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-900">Print / Cetak</button>
                    <button type="submit" onclick="setAction('{{ route('reportingpenjualandp.excel') }}')" class="px-5 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700">Export Excel</button>
