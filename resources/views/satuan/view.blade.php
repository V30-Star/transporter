    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
            <div class="px-4 pt-3 pb-0">
                <p class="text-xs font-medium uppercase tracking-wide text-gray-400">Identitas Satuan</p>
            </div>
            <div class="p-4 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Kode Satuan</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Nama Satuan</label>
                ...
                <div>
                    <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-not-allowed">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">Non-aktif menyembuyen Satuan dari transaksi baru</p>
                        </div>
                        <div class="relative w-9 h-5 duration-200 flex-shrink-0 cursor-not-allowed {{ $satuan->fnonactive == '0' ? 'bg-blue-500/60' : 'bg-gray-200' }}">
                            <div class="absolute w-3.5 h-3.5 bg-white rounded-full top-0.5 transition-transform duration-200 {{ $satuan->fnonactive == '0' ? 'translate-x-4 left-0.5' : 'left-0.5' }}"></div>
                        </div>
                    </div>
                </div>
        ...
        {{-- FOOTER INFO --}}
        @php
            $lastUpdate = $satuan->fupdatedat ?: $satuan->fcreatedat;
            $updatedBy = $satuan->fupdatedby ?: ($satuan->fcreatedby ?: '—');
        @endphp
