...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[700px] mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Satuan</h2>
            <p class="text-gray-600 mt-1">Data berikut akan dihapus secara permanen:</p>
        </div>

        <div class="bg-gray-50 rounded-lg p-6 space-y-4">
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Kode Satuan</div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Nama Satuan</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900 uppercase">
                    {{ $satuan->fsatuanname }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium
                        {{ $satuan->fnonactive == '1' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ $satuan->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>

            @if($satuan->fcreatedby)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Dibuat</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $satuan->fcreatedby ?? '-' }} - 
                    {{ $satuan->fcreatedat ? \Carbon\Carbon::parse($satuan->fcreatedat)->format('d/m/Y H:i') : '-' }}
                </div>
            </div>
            @endif

            @if($satuan->fupdatedat)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Diupdate</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $satuan->fupdatedby ?? '-' }} - 
                    {{ \Carbon\Carbon::parse($satuan->fupdatedat)->format('d/m/Y H:i') }}
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                        <li>Satuan sudah digunakan dalam data Product</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated: {{ $satuan->fupdatedat ? \Carbon\Carbon::parse($satuan->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($satuan->fcreatedat)->format('d M Y, H:i') }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus satuan <strong class="text-gray-900">{{ $satuan->fsatuancode }} - {{ $satuan->fsatuanname }}</strong>?
                </p>
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('satuan.destroy', $satuan->fsatuanid) }}', {
                method: 'POST',
                headers: {
