...
@section('title', $action === 'delete' ? 'Hapus Satuan' : 'Edit Satuan')
...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[1800px] w-full mx-auto">
        ...
        @if ($action === 'delete')
            ...
            {{-- ============================================ --}}
        @else
            <form action="{{ route('satuan.update', $satuan->fsatuanid) }}" method="POST" data-form-draft="true"
                data-draft-key="satuan:edit">
                @csrf
                @method('PATCH')
                ...
                @php
                    ...
                    $isUpdated = !empty($satuan->fupdatedat);
                @endphp
            </form>
        @endif
        <br>
        ...
        <span class="text-sm text-gray-600 flex justify-between items-center">
            ...
            <span>{{ \Carbon\Carbon::parse($satuan->fupdatedat ?: $satuan->fcreatedat)->timezone('Asia/Jakarta')->format('d M Y, H:i:s') }}</span>
        </span>
    </div>
    ...
    @if ($action === 'delete')
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">Konfirmasi hapus satuan ini?</h3>

                <form id="deleteForm" action="{{ route('satuan.destroy', $satuan->fsatuanid) }}" method="POST">
                    @csrf
                    @method('DELETE')
