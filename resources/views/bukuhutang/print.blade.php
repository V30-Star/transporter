...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .customer-heading { font-weight: bold; color: #c00; background-color: #fff; border: 1px solid #000; margin-top: 3px; padding: 3px; font-size: 8px; }
            ...
            @forelse ($rows->groupBy('faccount') as $account => $accountRows)
                ...
                @foreach ($accountRows->groupBy('fsupplier') as $supplier => $items)
                    @php
                        ...
                        $accountSaldo += $saldo;
                    @endphp
                    <div class="customer-heading">{{ $supplier }} - {{ $supplierName }}</div>
