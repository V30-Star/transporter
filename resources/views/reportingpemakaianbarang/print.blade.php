...
<head>
    ...
    <style>
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 28mm 18mm 24mm 1fr 24mm 14mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 20mm 1fr 20mm 20mm 14mm 18mm 20mm 20mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    @php
        ...
        foreach ($fakturpembelianData as $fakturpembelian) {
            $grandTotalAmount += (float) ($fakturpembelian->famount ?? 0);
        }

        $dateFromFmt = request('filter_date_from') ? date('d-m-Y', strtotime(request('filter_date_from'))) : '...';
        $dateToFmt = request('filter_date_to') ? date('d-m-Y', strtotime(request('filter_date_to'))) : '...';
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Pemakaian Barang</div>
                </div>
            </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 15px;">
                    ...
                    <div style="flex: 1; padding-right: 15px; border-right: 1px solid #000;">
                        <table class="info-col-table">
                            <tr>
                                <td class="info-col-label">Supplier/Gudang</td>
                                <td style="width: 8px;">:</td>
                                <td>{{ $activeSupplierName ?? 'Semua' }}</td>
                            </tr>
                            <tr>
                                <td class="info-col-label">Cabang</td>
                                <td>:</td>
                                <td>{{ request()->has('branch_codes') ? implode(', ', (array) request()->input('branch_codes')) : 'Semua' }}</td>
                            </tr>
                        </table>
                    </div>
                    {{-- Kanan --}}
        ...
        @foreach ($fakturpembelianData as $fakturpembelian)
            @php
                $viewUrl = route('pemakaianbarang.view', $fakturpembelian->fstockmtid);
                $editUrl = route('pemakaianbarang.edit', $fakturpembelian->fstockmtid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div class="truncate" title="{{ $fakturpembelian->fstockmtno }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $fakturpembelian->fstockmtno }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $fakturpembelian->fstockmtno }}</span>
                    </div>
                    <div>{{ \Carbon\Carbon::parse($fakturpembelian->fstockmtdate)->format('d-m-Y') }}</div>
                    <div class="truncate">{{ $fakturpembelian->fwhname }}</div>
                    ...
                    <div class="truncate">{{ $fakturpembelian->fusercreate }}</div>
                </div>
        ...
        <div class="po-totals-panel-wrapper">
            ...
            <div class="po-totals-container">
                <div class="po-total-row grand-total-row">
                    <span>TOTAL PEMAKAIAN BARANG</span>
                    <span>Rp {{ number_format((float) $grandTotalAmount, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="report-wrapper" id="reportWrapper">
        @if (empty($fakturpembelianData) || count($fakturpembelianData) === 0)
