                ...
                <div class="p-4 space-y-3">
                    <fieldset {{ in_array($action, ['view', 'delete'], true) ? 'disabled' : '' }} class="space-y-3">
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            {{-- Pembayaran, Nomor Retur, Nilai RP Retur --}}
                            <div class="col-span-3 grid grid-cols-3 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="frefretur" id="retailFrefretur" list="retailReturList"
                                    ...
                                    <datalist id="retailReturList"></datalist>
                                    @error('frefretur')
                                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <script>
                                    ...
                                    const returInput = document.getElementById('retailFrefretur');
                                    const amountInput = document.getElementById('retailFamountretur');
                                    const returList = document.getElementById('retailReturList');

                                    function loadRetailCustomerReturs(custNo) {
                                        if (!returList) return;
                                        returList.innerHTML = '';
                                        if (!custNo) return;
                                        fetch('{{ route('penjualanretail.customer-returs') }}?fcustno=' + encodeURIComponent(custNo))
                                            .then(r => r.json())
                                            .then(data => {
