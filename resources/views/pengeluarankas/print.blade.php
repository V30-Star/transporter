    ...
    <div id="raw-templates" style="display: none;">
        ...
        <div id="tpl-header">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">PENGELUARAN KAS/BANK</div>
                    <div class="so-no">No. {{ $hdr->fkasmtno ?? '-' }}</div>
                </div>
            </div>

            <div class="customer-container">
                <span class="customer-label">Informasi Pengeluaran</span>
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 10px;">
                    <div style="flex: 1; padding-right: 10px; border-right: 1px solid #000;">
                        <div style="font-size: 10px;">
                            <strong>Dibayar Kepada:</strong> {{ $hdr->fwhom ?: '-' }}
                        </div>
    ...
    <script>
        function runResponsivePagination() {
            ...
            for (let itemIdx = 0; itemIdx < itemPairs.length; itemIdx++) {
                ...
                if (isLastItem) {
                    ...
                    if (getContentHeight(currentSheet.sheet) > getMaxHeight(currentSheet.sheet)) {
                        currentSheet.footerSlot.innerHTML = '';
                        const contTest = tplContinued.cloneNode(true); contTest.removeAttribute('id');
            ...
            document.querySelectorAll('#print-container tbody tr.item-row-main').forEach(tr => {
                const cell = tr.querySelector('.row-no'); if (cell) cell.innerText = globalRow++;
            });
        }
