...
<head>
    ...
    <title>List of Master Customer</title>
    <style>
        ...
        * {
        ...
        /* --- TABLE HEADERS & ROWS (8 Kolom) --- */
        .po-header-labels,
        .po-header {
            display: grid;
            grid-template-columns: 8mm 20mm 30mm 1fr 22mm 22mm 24mm 24mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: start;
...
<body>
    @php
        ...
        $custRange = request('cust_from') || request('cust_to') ? (request('cust_from') ?: 'Awal') . ' s/d ' . (request('cust_to') ?: 'Akhir') : 'Semua';
        $salesmanText = request('salesman') ?: 'Semua';
        $limitText = request('limit') ? number_format((float) request('limit'), 0, ',', '.') : '0';
        $blockedText = request('is_blocked') ? 'Hanya Yang Diblokir' : 'Semua';
    @endphp
    ...
    <div id="raw-source" style="display: none;">
        ...
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">List of Master Customer</div>
                </div>
            </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 15px;">
                    ...
                    <div style="flex: 1; padding-right: 15px; border-right: 1px solid #000;">
                        <table class="info-col-table">
                            ...
                            <tr>
                                <td class="info-col-label">Limit Minimum</td>
                                <td>:</td>
                                <td>{{ $limitText }}</td>
                            </tr>
                            <tr>
                                <td class="info-col-label">Status Blokir</td>
                                <td>:</td>
                                <td>{{ $blockedText }}</td>
                            </tr>
                        </table>
        ...
        @foreach ($data as $i => $row)
            @php
                $viewUrl = route('customer.view', $row->fcustomerid);
                $editUrl = route('customer.edit', $row->fcustomerid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div>{{ $i + 1 }}</div>
                    <div class="truncate" title="{{ $row->fcustomercode }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $row->fcustomercode }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $row->fcustomercode }}</span>
                    </div>
                    ...
                    <div class="truncate" title="{{ $row->fsalesmanname ?? $row->fsalesman }}">{{ $row->fsalesmanname ?? $row->fsalesman }}</div>
                    <div style="line-height: 1.3; overflow: hidden;">
                        <span class="truncate" style="display: block;" title="{{ $row->fkontakperson }}">{{ $row->fkontakperson }}</span>
                        <small style="color: #64748b; display: block;" class="truncate" title="{{ $row->femail }}">{{ $row->femail }}</small>
                    </div>
