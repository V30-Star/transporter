...
@section('content')
    @php
        $canCreate = in_array('createDealer', explode(',', session('user_restricted_permissions', '')));
        $canEdit   = in_array('updateDealer', explode(',', session('user_restricted_permissions', '')));
        $canDelete = in_array('deleteDealer', explode(',', session('user_restricted_permissions', '')));
    ...
    <div class="bg-white rounded shadow p-4">
        ...
        <table id="dealerTable" class="min-w-full border text-sm">
            <thead class="bg-gray-100">
                <tr>
                    <th class="border px-2 py-2">Kode Dealer</th>
                    <th class="border px-2 py-2">Nama Dealer</th>
            ...
            <tbody>
                @foreach ($dealers as $item)
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="border px-2 py-1">{{ $item->fdealercode }}</td>
                        <td class="border px-2 py-1">{{ $item->fdealername }}</td>
                        ...
                        @if ($showActionsColumn)
                            <td class="border px-2 py-1 space-x-1.5 text-right">
                                <a href="{{ route('dealer.view', $item->fdealerid) }}">
                                ...
                                @if ($canEdit)
                                    <a href="{{ route('dealer.edit', $item->fdealerid) }}">
                                ...
                                @endif
                                @if ($canDelete)
                                    <a href="{{ route('dealer.delete', $item->fdealerid) }}"
