...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[96rem] mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Customer</h2>
            <p class="text-gray-600 mt-1">Data berikut akan dihapus secara permanen:</p>
        </div>

        <div class="bg-gray-50 rounded-lg p-6 space-y-4">
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Kode Customer</div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Nama Customer</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900 uppercase">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Tempo</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900">
                    {{ $customer->ftempo ?? 0 }} Hari
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium
                        {{ $customer->fnonactive == '1' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ $customer->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>

            @if($customer->fcreatedby)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Dibuat</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $customer->fcreatedby ?? '-' }} -
                    {{ $customer->fcreatedat ? \Carbon\Carbon::parse($customer->fcreatedat)->format('d/m/Y H:i') : '-' }}
                </div>
            </div>
            @endif

            @if($customer->fupdatedat)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Diupdate</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $customer->fupdatedby ?? '-' }} -
                    {{ \Carbon\Carbon::parse($customer->fupdatedat)->format('d/m/Y H:i') }}
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                        <li>Customer sudah digunakan dalam transaksi Sales Order</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated: {{ $customer->fupdatedat ? \Carbon\Carbon::parse($customer->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($customer->fcreatedat)->format('d M Y, H:i') }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus customer <strong class="text-gray-900">{{ $customer->fcustomercode }} - {{ $customer->fcustomername }}</strong>?
                </p>
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('customer.destroy', $customer->fcustomerid) }}', {
                method: 'POST',
                headers: {
