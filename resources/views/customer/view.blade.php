...
@section('title', 'Master Customer - View')
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <label
                                class="inline-flex items-center mt-5 font-medium text-xs text-gray-400 cursor-not-allowed">
                                <input type="checkbox" x-model="autoCode"
                                    class="form-checkbox h-4 w-4 rounded text-blue-500/60 border-gray-300 cursor-not-allowed"
                                    disabled>
                        ...
                        {{-- Nama Customer --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">Nama Customer</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select name="fgroup" id="groupSelect"
                                class="w-full bg-gray-100 text-gray-500 cursor-not-allowed" disabled>
                                <option value="">-- Pilih Group Customer --</option>
                                @foreach ($groups as $group)
                                    <option value="{{ $group->fgroupid }}"
                                        {{ $customer->fgroup == $group->fgroupid ? 'selected' : '' }}>
                                        {{ $group->fgroupname }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="fhargalevel" id="fhargalevel"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-gray-100 text-gray-500 cursor-not-allowed"
                                disabled>
                                <option value="0" {{ $customer->fhargalevel == 0 ? 'selected' : '' }}>Harga Level 1
                                </option>
                                <option value="1" {{ $customer->fhargalevel == 1 ? 'selected' : '' }}>Harga Level 2
                                </option>
                                <option value="2" {{ $customer->fhargalevel == 2 ? 'selected' : '' }}>Harga Level 3
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select name="fsalesman" id="salesmanSelect"
                                class="w-full bg-gray-100 text-gray-500 cursor-not-allowed" disabled>
                                <option value="">-- Pilih Salesman --</option>
                                @foreach ($salesman as $sales)
                                    <option value="{{ $sales->fsalesmancode }}"
                                        {{ $customer->fsalesman == $sales->fsalesmancode ? 'selected' : '' }}>
                                        {{ $sales->fsalesmanname }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="fwilayah" id="wilayahSelect"
                                class="w-full bg-gray-100 text-gray-500 cursor-not-allowed" disabled>
                                <option value="">-- Pilih Wilayah --</option>
                                @foreach ($wilayah as $wil)
                                    <option value="{{ $wil->fwilayahid }}"
                                        {{ $customer->fwilayah == $wil->fwilayahid ? 'selected' : '' }}>
                                        {{ $wil->fwilayahname }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                <div class="flex items-center gap-2 px-4 pt-3 pb-0">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 014 0m-3 8a3 3 0 100-6 3 3 0 000 6zm6-3h2m-2 4h2" />
                    </svg>
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        {{-- Nama KTP --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">Nama KTP</label>
                            <input type="text" value="{{ $customer->fnamaktp }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200"
                                readonly>
                        </div>
                    </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                <div class="flex items-center gap-2 px-4 pt-3 pb-0">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 7v12m0 0l-4-4m4 4l4-4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" />
                    ...
                    <div class="mt-4 border border-gray-200 rounded-lg p-3 bg-gray-50/50">
                        ...
                        <div class="flex space-x-1.5 mb-3">
                            <button type="button" @click="selectedAlamat = 'alamatsurat'"
                                :class="selectedAlamat === 'alamatsurat' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' :
                            ...
                            <button type="button" @click="selectedAlamat = 'alamatkirim'"
                                :class="selectedAlamat === 'alamatkirim' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' :
                                    'bg-white text-gray-600 hover:bg-gray-50 border-gray-200'"
                                class="px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors">
                                Alamat Kirim
                            </button>
                            <button type="button" @click="selectedAlamat = 'alamatpajak'"
                                :class="selectedAlamat === 'alamatpajak' ? 'bg-blue-600 text-white border-blue-600 shadow-sm' :
                                    'bg-white text-gray-600 hover:bg-gray-50 border-gray-200'"
                                class="px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors">
                                Alamat Pajak
                            </button>
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select name="fjadwaltukarfakturmingguan" id="fjadwaltukarfakturmingguan"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed"
                                disabled>
                                <option value="1"
                                    {{ $customer->fjadwaltukarfakturmingguan == '1' ? 'selected' : '' }}>Setiap Minggu
                                </option>
                                <option value="2"
                                    {{ $customer->fjadwaltukarfakturmingguan == '2' ? 'selected' : '' }}>Minggu Ganjil
                                </option>
                                <option value="3"
                                    {{ $customer->fjadwaltukarfakturmingguan == '3' ? 'selected' : '' }}>Minggu Genap
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="fjadwaltukarfakturhari" id="fjadwaltukarfakturhari"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed"
                                disabled>
                                <option value="">-- Pilih Hari --</option>
                                <option value="1" {{ $customer->fjadwaltukarfakturhari == '1' ? 'selected' : '' }}>
                                    Senin</option>
                                <option value="2" {{ $customer->fjadwaltukarfakturhari == '2' ? 'selected' : '' }}>
                                    Selasa</option>
                                <option value="3" {{ $customer->fjadwaltukarfakturhari == '3' ? 'selected' : '' }}>
                                    Rabu</option>
                                <option value="4" {{ $customer->fjadwaltukarfakturhari == '4' ? 'selected' : '' }}>
                                    Kamis</option>
                                <option value="5" {{ $customer->fjadwaltukarfakturhari == '5' ? 'selected' : '' }}>
                                    Jumat</option>
                                <option value="6" {{ $customer->fjadwaltukarfakturhari == '6' ? 'selected' : '' }}>
                                    Sabtu</option>
                                <option value="7" {{ $customer->fjadwaltukarfakturhari == '7' ? 'selected' : '' }}>
                                    Minggu</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="frekening" id="frekening"
                            class="w-full bg-gray-100 text-gray-500 cursor-not-allowed" disabled>
                            <option value="">Pilih Kode Rekening</option>
                            @foreach ($rekening as $rek)
                                <option value="{{ $rek->frekeningid }}"
                                    {{ $customer->frekening == $rek->frekeningid ? 'selected' : '' }}>
                                    {{ $rek->frekeningname }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            <div
                                class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-not-allowed">
                                <div>
                                    <p class="text-sm text-gray-800">Blokir Customer</p>
                                    <p class="text-xs text-gray-400 mt-0.5">Mencegah customer dari membuat transaksi baru
                                    </p>
                                </div>
                                <div
                                    class="relative w-9 h-5 rounded-full duration-200 flex-shrink-0 cursor-not-allowed {{ $customer->fblokir == '1' ? 'bg-red-500/60' : 'bg-gray-200' }}">
                                    <div
                                        class="absolute w-3.5 h-3.5 bg-white rounded-full top-0.5 transition-transform duration-200 {{ $customer->fblokir == '1' ? 'translate-x-4 left-0.5' : 'left-0.5' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5">Non-aktif menyembunyikan customer dari daftar
                                        aktif</p>
                                </div>
                                <div
                                    class="relative w-9 h-5 rounded-full duration-200 flex-shrink-0 cursor-not-allowed {{ $customer->fnonactive == '0' ? 'bg-blue-500/60' : 'bg-gray-200' }}">
                                    <div
                                        class="absolute w-3.5 h-3.5 bg-white rounded-full top-0.5 transition-transform duration-200 {{ $customer->fnonactive == '0' ? 'translate-x-4 left-0.5' : 'left-0.5' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
