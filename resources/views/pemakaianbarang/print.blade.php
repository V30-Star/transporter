    ...
    <div id="raw-templates" style="display: none;">
        ...
        <div id="tpl-header">
            ...
            <div class="customer-container">
                <span class="customer-label">Informasi Pemakaian</span>
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 10px;">
                    <div style="flex: 1; padding-right: 10px; border-right: 1px solid #000;">
                        <div style="font-size: 10px;">
                            <strong>Gudang:</strong> {{ $hdr->ffrom ?? '-' }} - {{ $hdr->fwhnamen ?? ($hdr->wh_name ?? '') }}
                        </div>
                        @if(!empty($hdr->faccount))
                            <div style="font-size: 10px; margin-top: 2px;">
                                <strong>Akun Biaya:</strong> {{ $hdr->faccount }} - {{ $hdr->faccname ?? '' }}
                            </div>
                        @endif
                    </div>
            ...
            <tbody id="raw-rows">
                @foreach ($dt as $i => $r)
                    @php
                        ...
                        $itemKet = !empty($r->account_name) ? $r->account_name : (!empty(trim((string)($r->fketdt ?? ''))) ? $r->fketdt : '-');
                    @endphp
