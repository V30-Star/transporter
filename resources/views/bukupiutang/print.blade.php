...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ledger-total {
            display: grid;
            grid-template-columns: 25mm 32mm 38mm 25mm 25mm 28mm;
            gap: 1px;
            font-size: 8px;
            padding: 3px;
        }
            ...
            @forelse ($rows->groupBy('faccount') as $account => $accountRows)
                ...
                @foreach ($accountRows->groupBy('fcustno') as $customer => $items)
                    @php
                        ...
                        $accountSaldo += $saldo;
                    @endphp
                    <div class="customer-heading">{{ $customer }} - {{ $customerName }}</div>
                    @foreach ($items as $row)
                        <div class="ledger-row">
                            ...
                            <div class="right">{{ number_format((float) $row->ppvarsaldo, 2, ',', '.') }}</div>
                        </div>
                    @endforeach
                    <div class="ledger-total">
                        <div style="grid-column: span 3;" class="right">Saldo Akhir {{ $customerName }}</div>
