...
<head>
    ...
    <style>
        * {
        ...
        .group-total-row {
            display: grid;
            grid-template-columns: 18mm 28mm 1fr 26mm 26mm 28mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .group-total-row > div:nth-child(4) {
            text-align: right;
        }
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Buku Piutang</div>
                </div>
            </div>
        ...
        @forelse ($rows->groupBy('faccount') as $account => $accountRows)
            ...
            <div class="journal-block group-row">
                Akun: {{ $account }} - {{ $accountName }}
            </div>

            @foreach ($accountRows->groupBy('fcustno') as $customer => $items)
                @php
                    $customerName = $items->first()->fcustname ?: $customer;
                ...
                @foreach ($items as $row)
                    ...
                    <div class="journal-block">
                        <div class="ledger-row">
                            ...
                            <div class="truncate {{ $isReturn ? 'text-rej' : '' }}" title="{{ $row->faccountno }}">
                                @if ($hasTrx)
                                    <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $row->faccountno }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $row->faccountno }}</span>
                                @else
                                    {{ $row->faccountno }}
                                @endif
                            </div>
                ...
                <div class="journal-block">
                    <div class="group-total-row">
                        <div style="color: #9a3412; grid-column: span 3; text-align: right; padding-right: 8px;">Saldo Akhir {{ $customerName }} :</div>
            ...
            {{-- Account Total block --}}
            <div class="journal-block">
                <div class="group-total-row account-total-row">
                    <div style="grid-column: span 3; text-align: right; padding-right: 8px;">Total {{ $accountName }} :</div>
            ...
            @php
                $grandDebit += $accountDebit;
                $grandCredit += $accountCredit;
                $grandSaldo += $accountSaldo;
            @endphp
        @empty
        @endforelse
    </div>
        ...
        <div class="po-totals-panel-wrapper">
            ...
            <div class="po-totals-container">
                <div class="po-total-row">
                    <span>GRAND TOTAL DEBET</span>
                    <span>Rp {{ number_format((float) $grandDebit, 2, ',', '.') }}</span>
                </div>
                <div class="po-total-row">
                    <span>GRAND TOTAL KREDIT</span>
                    <span>Rp {{ number_format((float) $grandCredit, 2, ',', '.') }}</span>
                </div>
                <div class="po-total-row grand-total-row">
                    <span>GRAND TOTAL SALDO AKHIR</span>
                    <span>Rp {{ number_format((float) $grandSaldo, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>
