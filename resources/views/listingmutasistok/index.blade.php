...
@section('content')
                            ...
                            <div>
                                ...
                                <select name="warehouse_from" class="select2 w-full border rounded px-3 py-2 text-sm">
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->fwhcode }}">{{ $warehouse->fwhcode }} -
                                            {{ $warehouse->fwhname }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
    ...
    <script>
        ...
        function selectAllBranches(status) {
            document.querySelectorAll('#branchCheckboxesArea .branch-checkbox:not(:disabled)').forEach(checkbox => checkbox
                .checked = status);
        }
