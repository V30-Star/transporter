...
@section('content')
    <div x-data class="bg-white rounded shadow p-4">
        ...
        <div class="flex justify-end items-center mb-4">
            ...
            @if ($canCreate)
                <a href="{{ route('tr_prh.create') }}"
                    @click="if (@js($createLimitReached)) { $event.preventDefault(); $store.tr_prhStore.openCreateLimitModal(); }"
        ...
        <div x-show="$store.tr_prhStore.showDeleteModal" x-cloak
            ...
            <div @click.away="!$store.tr_prhStore.isDeleting && $store.tr_prhStore.closeDelete()"
                class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">Konfirmasi Hapus</h3>
                <p class="mb-6">Apakah Anda yakin ingin menghapus data ini?</p>
                <div class="flex justify-end space-x-2">
                    <button @click="$store.tr_prhStore.closeDelete()"
                        class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400" :disabled="$store.tr_prhStore.isDeleting">
                        Batal
                    </button>
                    <button @click="$store.tr_prhStore.confirmDelete()"
                        class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        :disabled="$store.tr_prhStore.isDeleting">
                        <span x-show="!$store.tr_prhStore.isDeleting">Hapus</span>
                        <span x-show="$store.tr_prhStore.isDeleting">Menghapus...</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        {{-- Modal Create Limit --}}
        <div x-show="$store.tr_prhStore.showCreateLimitModal" x-cloak
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-transition>
            <div @click.away="$store.tr_prhStore.closeCreateLimitModal()" class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">Notifikasi</h3>
                <p class="mb-6">Batas membuat data sudah terlampaui</p>
                <div class="flex justify-end">
                    <button type="button" @click="$store.tr_prhStore.closeCreateLimitModal()"
        ...
        {{-- Toast Notification --}}
        <div x-show="$store.tr_prhStore.showNotification" x-cloak x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0 transform translate-y-2"
            x-transition:enter-end="opacity-100 transform translate-y-0"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed top-4 right-4 z-50 max-w-sm">
            <div :class="$store.tr_prhStore.notificationType === 'success' ? 'bg-green-500' : 'bg-red-500'"
                class="text-white px-6 py-4 rounded-lg shadow-lg flex items-center space-x-3">
                <span x-text="$store.tr_prhStore.notificationMessage"></span>
                <button @click="$store.tr_prhStore.showNotification = false" class="ml-4 text-white hover:text-gray-200">
                    ×
                </button>
            </div>
        </div>
...
@push('scripts')
    ...
    <script>
        ...
        $(function() {
            ...
            // Fungsi untuk menentukan apakah status approval mengizinkan edit atau tidak
            ...
            const table = $('#tr_prhTable').DataTable({
                ...
                initComplete: function() {
                    ...
                    $(document).on('click.tr_prh-column-search', function(e) {
