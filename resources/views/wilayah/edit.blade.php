...
@section('title', 'Edit - Master Wilayah')
...
<div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <form action="{{ route('wilayah.update', $wilayah->fwilayahid) }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="fwilayahcode" value="{{ old('fwilayahcode', $wilayah->fwilayahcode) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fwilayahcode') border-red-400 @enderror {{ !empty($isTransactionLocked) ? 'bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200' : '' }}"
                                {{ !empty($isTransactionLocked) ? 'readonly' : '' }} autofocus>
                            @if (!empty($isTransactionLocked))
                                <p class="text-[11px] text-amber-600 mt-1 font-medium">Kode wilayah dikunci karena sudah direferensi di transaksi.</p>
                            @endif
                            @error('fwilayahcode')
                        ...
                        <div>
                            ...
                            <input type="text" name="fwilayahname" value="{{ old('fwilayahname', $wilayah->fwilayahname) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fwilayahname') border-red-400 @enderror"
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden"
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div>
                        <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-pointer hover:border-gray-300 transition-colors"
                            ...
                            <div>
                                <p class="text-sm text-gray-800">Wilayah aktif</p>
                                <p class="text-xs text-gray-400 mt-0.5">Non-aktif menyembunyikan wilayah dari transaksi baru</p>
                            </div>
                ...
                <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-200">
                    <button type="button"
                        onclick="window.location.href='{{ route('wilayah.index') }}'"
        ...
        @php
            ...
            $updatedBy = $wilayah->fupdatedby ?: ($wilayah->fcreatedby ?: '—');
        @endphp
