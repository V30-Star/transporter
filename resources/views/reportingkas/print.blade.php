...
<body>
    <div class="no-print">
        ...
        @if(str_contains($pageTitle, 'Pengeluaran'))
            <a href="{{ route('reportingkas.pengeluaran.excel', request()->query()) }}"
        ...
        @else
            <a href="{{ route('reportingkas.penerimaan.excel', request()->query()) }}"
                style="padding: 7px 12px; background-color: #22c55e; color: white; border-radius: 4px; text-decoration: none; font-weight: bold; font-size: 12px; display: inline-flex; align-items: center; justify-content: center; transition: background-color 0.2s;"
                onmouseover="this.style.backgroundColor='#16a34a'"
                onmouseout="this.style.backgroundColor='#22c55e'">
                📊 Excel
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            ...
            <div class="info-tambahan">
                ...
                <div><span class="info-label">Opr</span>: {{ $user_session->fname ?? $user_session->name ?? '-' }}</div>
            </div>
        </div>
        ...
        @foreach ($records as $row)
            <div class="journal-block">
                <div class="po-header">
                    ...
                    <div>{{ $row->fnogiro ?: '-' }}</div>
                    <div class="truncate" title="{{ trim(($row->faccountheader ?? '') . ' - ' . ($row->header_account_name ?? ''), ' -') }}">{{ trim(($row->faccountheader ?? '') . ' - ' . ($row->header_account_name ?? ''), ' -') ?: '-' }}</div>
                    <div class="truncate" title="{{ $row->fket }}">{{ $row->fket ?: '-' }}</div>
                    <div>{{ number_format((float) ($row->famountpay ?? 0), 2, ',', '.') }}</div>
                </div>

                @if (!empty($detailsByHeader[$row->fkasmtid]) && $detailsByHeader[$row->fkasmtid]->count())
                    @foreach ($detailsByHeader[$row->fkasmtid] as $detail)
                        <div class="po-detail">
                            <div class="truncate" title="{{ trim(($detail->faccount ?? '') . ' - ' . ($detail->account_name ?? ''), ' -') }}">{{ trim(($detail->faccount ?? '') . ' - ' . ($detail->account_name ?? ''), ' -') ?: '-' }}</div>
                            <div class="truncate" title="{{ trim(($detail->fsubaccount ?? '') . ' - ' . ($detail->subaccount_name ?? ''), ' -') }}">{{ trim(($detail->fsubaccount ?? '') . ' - ' . ($detail->subaccount_name ?? ''), ' -') ?: '-' }}</div>
                            <div class="truncate" title="{{ $detail->fnote }}">{{ $detail->fnote ?: '-' }}</div>
                            <div>{{ number_format((float) ($detail->fkasdtvalue ?? 0), 2, ',', '.') }}</div>
                            <div>{{ $detail->fdk ?: '-' }}</div>
                        </div>
                    @endforeach
                @endif
