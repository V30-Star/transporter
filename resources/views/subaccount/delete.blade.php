...
@section('title', 'Hapus Subaccount')
...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[1800px] w-full mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Subaccount</h2>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span
                        ...
                        {{ $subaccount->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated:
                {{ $subaccount->fupdatedat ? \Carbon\Carbon::parse($subaccount->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($subaccount->fcreatedat)->format('d M Y, H:i') }}
            </p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus subaccount <strong
                        class="text-gray-900">{{ $subaccount->fsubaccountcode }} -
                        {{ $subaccount->fsubaccountname }}</strong>?
                </p>
    ...
    <div x-data="notificationApp()" x-show="showNotification" x-cloak x-transition:enter="transition ease-out duration-300"
        ...
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed top-4 right-4 z-50 max-w-sm">
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('subaccount.destroy', $subaccount->fsubaccountid) }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    ...
                    if (data.success || data.redirect) {
                        setTimeout(() => {
                            window.location.href = '{{ route('subaccount.index') }}';
                        }, 1000);
                    }
                })
                .catch(error => {
