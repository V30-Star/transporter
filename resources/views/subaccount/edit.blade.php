...
@section('title', 'Edit - Master Subaccount')
...
<div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <form action="{{ route('subaccount.update', $subaccount->fsubaccountid) }}" method="POST" id="formSubaccount">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                    ...
                    {{-- Kode & Nama Subaccount (2 kolom) --}}
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">
                                Kode Subaccount <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="fsubaccountcode" id="fsubaccountcode"
                                value="{{ old('fsubaccountcode', $subaccount->fsubaccountcode) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fsubaccountcode') border-red-400 @enderror"
                                maxlength="10" placeholder="cth. SUB-001" autofocus>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">
                                Nama Subaccount <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="fsubaccountname" id="fsubaccountname"
                                value="{{ old('fsubaccountname', $subaccount->fsubaccountname) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fsubaccountname') border-red-400 @enderror"
                                maxlength="50" placeholder="cth. Sub Kas">
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                ...
                <div class="p-4 space-y-4">
                    ...
                    {{-- Status Aktif --}}
                    <div x-data="{ active: {{ old('fnonactive', $subaccount->fnonactive) == '1' ? 'false' : 'true' }} }">
                        <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-pointer hover:border-gray-300 transition-colors"
                            ...
                            <div>
                                <p class="text-sm text-gray-800">Subaccount aktif</p>
                                <p class="text-xs text-gray-400 mt-0.5">Non-aktif menyembunyikan subaccount dari transaksi baru</p>
                            </div>
                ...
                <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-200">
                    <button type="button"
                        onclick="window.location.href='{{ route('subaccount.index') }}'"
        ...
        {{-- FOOTER INFO --}}
        @php
            $lastUpdate = $subaccount->fupdatedat ?: $subaccount->fcreatedat;
            $updatedBy = $subaccount->fupdatedby ?: ($subaccount->fcreatedby ?: '—');
        @endphp
