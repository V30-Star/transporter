...
@section('title', $action === 'delete' ? 'Hapus Sub Account' : 'Edit Sub Account')

@section('content')
    <div class="bg-white rounded shadow p-6  md:p-8 max-w-[700px] mx-auto">
        {{-- HEADER --}}
        <h2 class="text-2xl font-bold mb-6 text-center">
            {{ $action === 'delete' ? 'Data Sub Account' : 'Edit Sub Account' }}
        </h2>
        ...
        @else
            <form action="{{ route('subaccount.update', $subaccount->fsubaccountid) }}" method="POST">
                ...
                @php
                    ...
                    $isUpdated = !empty($subaccount->fupdatedat);
                @endphp
    ...
    @if ($action === 'delete')
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                ...
                <p class="mb-2 font-medium">Hapus subaccount ini?</p>
                <p class="mb-1 text-sm text-gray-600">Kode: <strong>{{ $subaccount->fsubaccountcode }}</strong></p>
                <p class="mb-6 text-sm text-gray-600">Nama: <strong>{{ $subaccount->fsubaccountname }}</strong></p>
