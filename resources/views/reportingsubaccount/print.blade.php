...
<head>
    ...
    <style>
        ...
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 10mm 28mm 1fr 20mm 25mm 25mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    @php
        ...
        $subaccountRange = request('subaccount_from') || request('subaccount_to') ? (request('subaccount_from') ?: 'Awal') . ' s/d ' . (request('subaccount_to') ?: 'Akhir') : 'Semua';
    @endphp
    ...
    <div id="raw-source" style="display: none;">
        ...
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Master Sub Account Report</div>
                </div>
            </div>
        ...
        @foreach ($data as $i => $row)
            @php
                $viewUrl = route('subaccount.view', $row->fsubaccountid);
                $editUrl = route('subaccount.edit', $row->fsubaccountid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div>{{ $i + 1 }}</div>
                    <div class="truncate" title="{{ $row->fsubaccountcode }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $row->fsubaccountcode }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $row->fsubaccountcode }}</span>
                    </div>
        ...
        <div class="trx-modal-card" onclick="event.stopPropagation()">
            <div class="trx-modal-header">
                <div class="trx-modal-title">
                    📄 Sub Account <strong id="modalTrxNo"></strong>
                </div>
                <button type="button" class="trx-modal-close" onclick="closeTrxActionModal()">&times;</button>
            </div>
            <div class="trx-modal-body">
                <p class="trx-modal-desc">Pilih tindakan untuk data sub account ini:</p>
