...
@section('title', 'Edit - Master Salesman')
...
<div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <form action="{{ route('salesman.update', $salesman->fsalesmanid) }}" method="POST" data-form-draft="true" data-draft-key="salesman:edit">
            @csrf
            @method('PATCH')
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="fsalesmancode" value="{{ old('fsalesmancode', $salesman->fsalesmancode) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fsalesmancode') border-red-400 @enderror {{ !empty($isTransactionLocked) ? 'bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200' : '' }}"
                                {{ !empty($isTransactionLocked) ? 'readonly' : '' }} autofocus>
                            @if (!empty($isTransactionLocked))
                                <p class="text-[11px] text-amber-600 mt-1 font-medium">Kode salesman dikunci karena sudah direferensi di transaksi.</p>
                            @endif
                            @error('fsalesmancode')
            ...
            {{-- ─── CARD 2: Status ────────────────────────────────────── --}}
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden"
                x-data="{ active: {{ old('fnonactive', $salesman->fnonactive) == '1' ? 'false' : 'true' }} }">
                <div class="px-4 pt-3 pb-0">
