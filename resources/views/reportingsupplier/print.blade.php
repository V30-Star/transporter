...
<head>
    ...
    <title>List of Master Supplier</title>
    <style>
        ...
        * {
        ...
        /* --- TABLE HEADERS & ROWS (7 Kolom) --- */
        .po-header-labels,
        .po-header {
            display: grid;
            grid-template-columns: 8mm 22mm 35mm 1fr 24mm 25mm 18mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: start;
...
<body>
    @php
        ...
        $supplierRange = request('supplier_from') || request('supplier_to') ? (request('supplier_from') ?: 'Awal') . ' s/d ' . (request('supplier_to') ?: 'Akhir') : 'Semua';
    @endphp
    ...
    <div id="raw-source" style="display: none;">
        ...
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">List of Master Supplier</div>
                </div>
            </div>
        ...
        @foreach ($data as $i => $row)
            @php
                $viewUrl = route('supplier.view', $row->fsupplierid);
                $editUrl = route('supplier.edit', $row->fsupplierid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div>{{ $i + 1 }}</div>
                    <div class="truncate" title="{{ $row->fsuppliercode }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $row->fsuppliercode }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $row->fsuppliercode }}</span>
                    </div>
                    ...
                    <div class="truncate" title="{{ $row->fkontakperson }}">{{ $row->fkontakperson }}</div>
                    <div>{{ $row->ftempo ?? '0' }} Hari</div>
                </div>
            </div>
        @endforeach
    </div>
