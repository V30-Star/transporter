...
<head>
    ...
    <title>Listing Penerimaan Kas/Bank</title>
    <!-- Fonts -->
    ...
    <style>
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 28mm 18mm 38mm 1fr 24mm 14mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 28mm 48mm 1fr 24mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    ...
    <div class="no-print">
        ...
        <a href="{{ route('listingpenerimaankasbank.excel', request()->query()) }}"
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Penerimaan Kas/Bank</div>
                </div>
            </div>
                    ...
                    <div style="flex: 1; padding-right: 15px; border-right: 1px solid #000;">
                        <table class="info-col-table">
                            <tr>
                                ...
                                <td>{{ request('account_no') ?: 'Semua' }}</td>
                            </tr>
                            <tr>
                                <td class="info-col-label">Giro Mundur</td>
                                <td>:</td>
                                <td>{{ request('giro_mundur') ? 'Ya' : 'Tidak' }}</td>
                            </tr>
                            <tr>
        ...
        <div class="po-header-labels">
            ...
            <div>Keterangan</div>
            <div style="text-align: right;">Total Bayar</div>
            <div>User-id</div>
        </div>
        ...
        <div class="po-detail-labels">
            ...
            <div>Uraian</div>
            <div style="text-align: right;">Nilai Bayar</div>
        </div>

        @foreach ($grouped as $items)
            @php
                $h = $items->first();
                $viewUrl = route('penerimaankas.view', $h->fkasmtno);
                $editUrl = route('penerimaankas.edit', $h->fkasmtno);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div class="truncate" title="{{ $h->fkasmtno }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $h->fkasmtno }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $h->fkasmtno }}</span>
                    </div>
        ...
        <div class="po-totals-panel-wrapper">
            ...
            <div class="po-totals-container">
                <div class="po-total-row grand-total-row">
                    <span>TOTAL PENERIMAAN KAS/BANK</span>
