@props([
    ...
    'controlsId' => 'warehouseTableControls',
...
<div x-data="warehouseBrowser()" x-init="init()" x-show="open" x-cloak x-transition.opacity
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-6xl flex flex-col overflow-hidden"
        ...
        <div
            ...
            <div>
                <h3 class="text-xl font-bold text-gray-800">{{ __('ui.browse_warehouse') }}</h3>
                <p class="text-sm text-gray-500 mt-0.5">{{ __('ui.select_warehouse_prompt') }}</p>
            </div>
                ...
                <table id="{{ $tableId }}" class="min-w-full text-sm display nowrap stripe hover" style="width:100%">
                    <thead class="sticky top-0 z-10">
                        <tr class="bg-gradient-to-r from-gray-50 to-gray-100">
                            <th class="text-left p-3 font-semibold text-gray-700 border-b-2 border-gray-200">{{ __('ui.warehouse') }}
                                ({{ __('ui.code') }} - {{ __('ui.name') }})</th>
                            <th class="text-left p-3 font-semibold text-gray-700 border-b-2 border-gray-200">{{ __('ui.branch') }}</th>
