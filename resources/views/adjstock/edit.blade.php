                                        ...
                                        <table class="adjstock-detail-table min-w-full text-sm balanced-detail-table"
                                            ...
                                            <thead class="bg-gray-100">
                                                <tr>
                                                    ...
                                                    <th class="p-2 text-left w-[27rem]">Nama Produk</th>
        ...
        <form action="{{ route('adjstock.update', $adjstock->fstockmtid) }}" method="POST" class="mt-6"
                        ...
                        <div x-data="{ autoCode: true }">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="fstockmtno"
                                    ...
                                    value="{{ strtoupper(old('fstockmtno', $adjstock->fstockmtno ?? '')) }}"
                                    :disabled="autoCode"
                                    :required="!autoCode"
                                ...
                                <label class="inline-flex items-center select-none text-xs text-gray-600 font-medium cursor-pointer">
            ...
            <div x-data="prhFormModal()" class="mt-3">
                <div class="mt-3 flex justify-between items-start gap-4">
                    ...
                    <div x-show="showDupModal" x-cloak class="fixed inset-0 z-[95] flex items-center justify-center"
                        x-transition.opacity>
                        <!-- Backdrop -->
    ...
    @if ($action === 'delete' && $canDeletePermission)
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 allow-action">
                <h3 class="text-lg font-semibold mb-4">Hapus Adjustment Stock ini?</h3>
        ...
        <script>
            ...
            function confirmDelete(forceSave = false) {
                ...
                fetch('{{ route('adjstock.destroy', $adjstock->fstockmtid) }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(result => {
                        if (result.ok) {
                            ...
                            setTimeout(() => {
                                window.location.href = '{{ route('adjstock.index') }}';
                            }, 500);
            ...
            addManyFromPR(header, items) {
                ...
                items.forEach(src => {
                    const row = {
                        ...
                        fqty: (src.fqty !== null && src.fqty !== undefined && (@json(stock_boleh_minus()) ? Number(src.fqty) !== 0 : Number(src.fqty) > 0)) ? Number(src.fqty) : 1,
