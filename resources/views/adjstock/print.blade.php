...
<head>
    <meta charset="utf-8">
    <title>Penyesuaian Stok - {{ $hdr->fstockmtno ?? '-' }}</title>
    ...
    <div id="raw-templates" style="display: none;">
        ...
        <div id="tpl-header">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">PENYESUAIAN STOK</div>
                    <div class="so-no">No. {{ $hdr->fstockmtno ?? '-' }}</div>
                </div>
            </div>

            <div class="customer-container">
                <span class="customer-label">Informasi Penyesuaian</span>
        ...
        <table id="tpl-table">
            <thead id="tpl-thead">
                <tr>
                    ...
                    <th class="text-right" style="width: 60px;">Qty Selisih</th>
...
<body>
    ...
    @php
        $isMasuk = strtoupper(trim((string) ($hdr->ftrancode ?? 'M'))) !== 'K';
        $calcTotalHarga = 0;
        foreach ($dt as $r) {
            $qty = (float) ($r->fqty ?? 0);
        ...
        $displayGrandTotal = (float) ($hdr->famountmt ?? ($hdr->famount ?? 0)) > 0 ? (float) ($hdr->famountmt ?? $hdr->famount) : $calcTotalHarga;
    @endphp
