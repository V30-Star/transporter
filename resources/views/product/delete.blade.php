...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[96rem] mx-auto">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span
                        ...
                        {{ $product->fnonactive == '1' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ $product->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>
                    ...
                    <div class="flex flex-col gap-6">
                        @foreach ([1, 2, 3] as $imgNo)
                            ...
                            <div class="border-b border-gray-100 pb-4 last:border-0">
                                ...
                                @else
                                    <div
                                        class="max-w-xs h-20 border rounded bg-gray-100 text-gray-400 flex items-center justify-center text-xs italic">
                    ...
                    <div>
                        ...
                        <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                            @foreach ($usedByLabels as $usedBy)
                                <li>Produk sudah direferensi di {{ strtolower($usedBy) }}</li>
                            @endforeach
                        </ul>
                    </div>
                </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated:
                {{ $product->fupdatedat ? \Carbon\Carbon::parse($product->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($product->fcreatedat)->format('d M Y, H:i') }}
            </p>
        </div>
    </div>
    ...
    @if (!$hasRelatedData)
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-gray-700 mb-4">
                        Apakah Anda yakin ingin menghapus product <strong class="text-gray-900">{{ $product->fprdcode }} -
                            {{ $product->fprdname }}</strong>?
                    </p>
        ...
        <div x-data="notificationApp()" x-show="showNotification" x-cloak
