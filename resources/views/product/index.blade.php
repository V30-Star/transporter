                                    ...
                                    <template x-for="(item, i) in $store.laporanStore.customerData" :key="i">
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 whitespace-nowrap text-slate-900">
                                                <a :href="(['REJ','RUJ'].includes(item.ftrcode) ? '{{ route('returpenjualan.index') }}' : '{{ route('invoice.index') }}') + '/' + encodeURIComponent(item.fsono) + '/view'" target="_blank" class="text-blue-600 hover:text-blue-800 underline font-medium" x-text="item.fsono"></a>
                                            </td>
                                            <td class="px-3 py-2 whitespace-nowrap text-slate-900"
            ...
            if (hasActions) {
                columns.push({
                    ...
                    render: function(fprdid, type, row) {
                        ...
                        html += `
                            <button onclick="openLaporanModal('${fprdid}')" class="inline-flex items-center bg-indigo-500 text-white px-3 py-1.5 text-xs rounded hover:bg-indigo-600">
                                <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        if (canEdit) {
                            if (editBlocked) {
                                html += `
                                    <button type="button" onclick="showProductApprovalLocked()" class="inline-flex items-center bg-yellow-500 text-white px-3 py-1.5 text-xs rounded hover:bg-yellow-600">
                                        <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
