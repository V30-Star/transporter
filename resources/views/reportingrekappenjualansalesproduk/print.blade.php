...
<head>
    ...
    <style>
        ...
        .group { font-weight: bold; background: #f9fafb; padding: 4px 6px; }
...
<body>
    <div class="actions">
        ...
        <a href="{{ route('reportingrekappenjualansalesproduk.excel', request()->query()) }}">Export Excel</a>
    </div>

    <div class="page">
        <h1>PT. UTALIYA</h1>
        <h2>Rekap Penjualan Sales By Produk</h2>
        ...
        <div class="meta">
            ...
            <div>Merek: {{ $filters['merek_label'] }}</div>
            <div>Group Produk: {{ $filters['group_label'] }}</div>
        </div>

        @php $grand = 0; @endphp
        @foreach ($rows->groupBy('fcustno') as $salesCode => $salesRows)
            @php $salesTotal = 0; $salesQtyBesar = 0; $salesQtyKecil = 0; @endphp
            <div class="group">Salesman: {{ $salesCode }} {{ $salesRows->first()->salesman_name }}</div>
            ...
            <table>
                <tr class="subtotal">
                    <td colspan="2">TOTAL {{ $salesRows->first()->salesman_name ?: $salesCode }}</td>
                    <td class="num">{{ number_format($salesQtyBesar, 2, ',', '.') }}</td>
                    <td class="num">{{ number_format($salesQtyKecil, 2, ',', '.') }}</td>
