...
<head>
    ...
    <style>
        * {
        ...
        /* --- PI HEADER STYLES (9 Kolom) --- */
        .po-header-labels,
        .po-header {
            display: grid;
            grid-template-columns: 10mm 25mm 18mm 12mm 1fr 20mm 16mm 22mm 12mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        /* --- PI DETAIL STYLES (8 Kolom) --- */
        .po-detail-labels,
        .po-detail {
            display: grid;
            grid-template-columns: 20mm 1fr 20mm 16mm 10mm 18mm 16mm 22mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    @php
        ...
        foreach ($groupedData as $fstockmtno => $details) {
            ...
            $totalPajak += (float) $h->famountpajak;
            $totalFaktur += (float) $h->famountmt;
        }
        ...
        $typeText = request('type_transaksi') ?: 'Semua';
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Faktur Pembelian</div>
                </div>
            </div>
        ...
        <div class="po-header-labels">
            ...
            <div style="text-align: right;">Total Faktur</div>
            <div>User-Id</div>
        </div>
        ...
        <div class="po-detail-labels">
            ...
            <div style="text-align: center;">Adj</div>
        ...
        @foreach ($groupedData as $fstockmtno => $details)
            @php
                $h = $details->first();
                $viewUrl = route('fakturpembelian.view', $h->fstockmtid);
                $editUrl = route('fakturpembelian.edit', $h->fstockmtid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    ...
                    <div>{{ number_format((float) $h->famount, 2, ',', '.') }}</div>
                    <div>{{ number_format((float) $h->famountpajak, 2, ',', '.') }}</div>
                    <div>{{ number_format((float) $h->famountmt, 2, ',', '.') }}</div>
                ...
                @foreach ($details as $d)
                    @if (!empty($d->fprdcode))
                        <div class="po-detail">
                            ...
                            <div>{{ number_format((float) $d->fcostother, 2, ',', '.') }}</div>
                            <div>{{ number_format((float) $d->ftotprice, 2, ',', '.') }}</div>
                        </div>
                    @endif
                @endforeach
        ...
        <div class="po-totals-panel-wrapper">
            ...
            <div class="po-totals-container">
                <div class="po-total-row">
                    ...
                    <span>Rp {{ number_format((float) $totalHarga, 2, ',', '.') }}</span>
                </div>
                <div class="po-total-row">
                    <span>TOTAL PPN</span>
                    <span>Rp {{ number_format((float) $totalPajak, 2, ',', '.') }}</span>
                </div>
