...
<head>
    ...
    <style>
        * {
        ...
        /* --- PR HEADER STYLES (7 Kolom) --- */
        .po-header-labels,
        .po-header {
            display: grid;
            grid-template-columns: 15mm 30mm 20mm 1fr 24mm 24mm 14mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 2px 6px;
            align-items: center;
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 30mm 1fr 20mm 24mm 24mm;
            gap: 1px;
            font-size: 8px;
            padding: 2px 6px;
            align-items: center;
...
<body>
    ...
    <div class="no-print">
        ...
        {{-- Excel Download --}}
        <a href="{{ route('listingpr.excel', request()->all()) }}"
    ...
    <div id="raw-source" style="display: none;">
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Listing Purchase Request</div>
                </div>
            </div>
        ...
        @foreach ($groupedData as $fprno => $details)
            @php
                $h = $details->first();
                $headerTotalQty = $details->sum('fqty');
                $viewUrl = route('tr_prh.view', $h->fprhid);
                $editUrl = route('tr_prh.edit', $h->fprhid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    <div>{{ $h->fbranchcode }}</div>
                    <div class="truncate" title="{{ $h->fprno }}">
                        <span class="trx-action-trigger" onclick="openTrxActionModal(event, '{{ $h->fprno }}', '{{ $viewUrl }}', '{{ $editUrl }}')">{{ $h->fprno }}</span>
                    </div>
                    <div>{{ \Carbon\Carbon::parse($h->fprdate)->format('d-m-Y') }}</div>
                    <div class="truncate" title="{{ $h->fsuppliername }}">{{ $h->fsuppliername }}</div>
                    <div>{{ \Carbon\Carbon::parse($h->fneeddate)->format('d-m-Y') }}</div>
                    <div>{{ number_format($headerTotalQty, 2, ',', '.') }}</div>
                    ...
                    @foreach ($details as $d)
                        @if (!empty($d->fitemcode))
                            <div class="po-detail">
                                ...
                                <div>{{ number_format((float) $d->fqtypo, 2, ',', '.') }}</div>
                            </div>
                        @endif
                    @endforeach
                @endif
