    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-4">
                <h3 class="text-xl font-bold text-gray-800">Buku Besar</h3>
                <button onclick="toggleModal(false)" class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
            </div>

            <form method="GET" action="{{ route('bukubesar.print') }}" target="_blank">
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold uppercase mb-1">Account Dari</label>
                            <select name="account_from" class="select2 w-full">
                                <option value="">-- Awal --</option>
                                @foreach ($accounts as $account)
                                    <option value="{{ $account->faccount }}">{{ $account->faccount }} - {{ $account->faccname }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold uppercase mb-1">Sampai</label>
                            <select name="account_to" class="select2 w-full">
                    ...
                    <div>
                        ...
                        <select name="sort_by" class="w-full border rounded px-3 py-2 text-sm" disabled>
                            <option value="invoice" selected>No.Invoice</option>
                        </select>
                    </div>
                </div>
