        ...
        <table id="tpl-table">
            <thead id="tpl-thead">
                <tr>
                    ...
                    <th>No. Nota / Tgl. Nota</th>
                    <th class="text-right" style="width: 70px;">Nilai Nota</th>
                    <th class="text-right" style="width: 60px;">Ongkos</th>
                    <th class="text-right" style="width: 72px;">Sisa Piutang</th>
                </tr>
            </thead>
            <tbody id="raw-rows">
                @foreach ($dt as $i => $r)
                    {{-- Row 1: No & No Nota + Tanggal --}}
                    <tr class="item-row item-row-main">
                        <td class="text-center row-no">{{ $i + 1 }}</td>
                        <td colspan="4" style="font-weight: bold;">
                            {{ $r->frefsono ?? '-' }} @if(!empty($r->frefcode)) ({{ $r->frefcode }}) @endif
                            @if(!empty($r->fsodate))
                                <span style="font-weight: normal; font-size: 9px; color: #444; margin-left: 6px;">
                                    (Tgl: {{ \Carbon\Carbon::parse($r->fsodate)->locale('id')->translatedFormat('d-m-Y') }})
                                </span>
                            @endif
                        </td>
                    </tr>
                    {{-- Row 2: Nilai Nota, Ongkos, Sisa Piutang --}}
            ...
            <div class="summary-container">
                ...
                <div style="width: 36%; padding: 0 6px;">
                    <div style="font-size: 9px; color: #333; font-style: italic;">
                        Terbilang:<br>
                        <strong># {{ strtoupper(terbilang($famounttagihan)) }} RUPIAH #</strong>
                    </div>
                </div>
