...
@section('content')
                ...
                <div class="p-4 space-y-3">
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <div x-data="{ autoTax: {{ trim((string) $invoice->ftaxno) === trim((string) $invoice->fsono) ? 'true' : 'false' }} }">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="ftaxno" value="{{ old('ftaxno', $invoice->ftaxno) }}"
                                    class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200"
                                    readonly>

                                <label class="inline-flex items-center select-none font-medium text-sm text-gray-600">
                        ...
                        <div class="col-span-3">
                            ...
                            <input type="text" value="{{ trim((string) ($invoice->fpembayaran ?? '')) }}" disabled
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">
                        </div>
                        ...
                        <div class="col-span-3">
                            ...
                            <input type="text" value="{{ trim((string) ($invoice->frefretur ?? '')) }}" disabled
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">
                        </div>
                        ...
                        <div class="col-span-3">
                            ...
                            <input type="text" value="{{ ($invoice->famountretur ?? 0) > 0 ? number_format((float) $invoice->famountretur, 2, ',', '.') : '0,00' }}" disabled
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-right bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200">
                        </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                                ...
                                @endphp
                                <div x-data="{ autoTax: {{ old('_token') !== null ? (old('ftax_auto') == '1' ? 'true' : 'false') : ($isAutoTax ? 'true' : 'false') }} }">
                                    <label class="block text-xs font-bold mb-1">Faktur Pajak#</label>
                                    <div class="flex items-center gap-2">
                                        <input type="text" id="ftaxno" name="ftaxno"
                                            ...
                                            :class="(autoTax || '{{ $action }}'
                                                === 'view') ? 'bg-gray-100 text-gray-500 border-gray-200 cursor-not-allowed' :
                                            'bg-white'"
 ...
 <div>
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    ...
                                    <div class="col-span-3">
                                        ...
                                        <select name="fpembayaran" id="invoiceFpembayaran"
                                            ...
                                            @foreach ($typePembayarans ?? [] as $tp)
                                                ...
                                                @endphp
                                                <option value="{{ $tpName }}" {{ $currentPembayaran === $tpName ? 'selected' : '' }}>
                                                    {{ $tpName }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-span-3">
                                        <label class="block text-xs font-bold mb-1">Nomor Retur</label>
                                        <select name="frefretur" id="invoiceFrefretur"
                                ...
                                <script>
                                    document.addEventListener('DOMContentLoaded', function() {
                                        ...
                                        const returSelect = document.getElementById('invoiceFrefretur');
                                        ...
                                        function loadInvoiceCustomerReturs(custNo) {
                                            ...
                                            fetch('{{ route('invoice.customer-returs') }}?fcustno=' + encodeURIComponent(custNo) + '&current_retur=' + encodeURIComponent(currentSelected))
                                                .then(r => r.json())
                                                .then(data => {
...
<script>
    ...
    window.validateAndSubmitInvoiceForm = function(form, isViewMode = false) {
        ...
        const returSelectEl = document.getElementById('invoiceFrefretur');
        const amountInputEl = document.getElementById('invoiceFamountretur');
        ...
        return {
            ...
            addManyFromPR(header, items, source = 'SO') {
                ...
                items.forEach(src => {
                    ...
                    const remainingQty = Math.max(0, Number(src.fqtyremain ?? 0));
