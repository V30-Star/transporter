...
<head>
    ...
    <title>Master Product List</title>
    <style>
        ...
        * {
        ...
        .po-header-labels > div:nth-child(n+4),
        .po-header > div:nth-child(n+4) {
            text-align: right;
        }
...
<body>
    @php
        ...
        $prdRange = request('prd_from') || request('prd_to') ? (request('prd_from') ?: 'Awal') . ' s/d ' . (request('prd_to') ?: 'Akhir') : 'Semua';
        $groupText = request('group') ?: 'Semua';
        $merekText = request('merek') ?: 'Semua';
        $sortText = request('sort_by') === 'name' ? 'Nama Produk' : 'Kode Produk';
        $onlyStockText = request('only_stock') ? 'Ya' : 'Tidak';
        $kolomList = collect(['hpp' => 'HPP', 'price1' => 'Jual 1', 'price2' => 'Jual 2', 'price3' => 'Jual 3'])->filter(fn($lbl, $key) => $showCols[$key] ?? false)->values()->all();
        $kolomText = !empty($kolomList) ? implode(', ', $kolomList) : '-';
    @endphp
    ...
    <div id="raw-source" style="display: none;">
        ...
        <div class="header-section">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">Master Product List</div>
                </div>
            </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 15px;">
                    ...
                    <div style="flex: 1; padding-right: 15px; border-right: 1px solid #000;">
                        <table class="info-col-table">
                            ...
                            <tr>
                                <td class="info-col-label">Group</td>
                                <td>:</td>
                                <td>{{ $groupText }}</td>
                            </tr>
                            <tr>
                    ...
                    <div style="flex: 1; padding-left: 5px;">
                        <table class="info-col-table">
                            ...
                            <tr>
                                <td class="info-col-label">Kolom Tampil</td>
                                <td>:</td>
                                <td>{{ $kolomText }}</td>
                            </tr>
                            <tr>
        ...
        @foreach ($data as $i => $row)
            @php
                $viewUrl = route('product.view', $row->fprdid);
                $editUrl = route('product.edit', $row->fprdid);
            @endphp
            <div class="journal-block">
                <div class="po-header">
                    ...
                    @endif
                    @if ($showCols['price1'])
                        <div>{{ number_format((float) $row->fhargajuallevel1, 2, ',', '.') }}</div>
                    @endif
                    @if ($showCols['price2'])
                        <div>{{ number_format((float) $row->fhargajuallevel2, 2, ',', '.') }}</div>
                    @endif
                    @if ($showCols['price3'])
                        <div>{{ number_format((float) $row->fhargajuallevel3, 2, ',', '.') }}</div>
                    @endif
                </div>
            </div>
        @endforeach
