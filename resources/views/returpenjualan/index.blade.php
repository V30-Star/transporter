...
@section('content')
    <div x-data class="bg-white rounded shadow p-4">
        ...
        @php
            ...
            $canCreate = in_array('createReturPenjualan', $permissions, true);
            $canEdit = in_array('updateReturPenjualan', $permissions, true);
            $canDelete = in_array('deleteReturPenjualan', $permissions, true);
        ...
        <div class="flex justify-end items-center mb-4">
            <div></div>
            @if ($canCreate)
                <a href="{{ route('returpenjualan.create') }}"
                    @click="if (@js($createLimitReached)) { $event.preventDefault(); $store.trsomtStore.openCreateLimitModal(); }"
                    ...
                    <th class="border px-2 py-1">
                        ...
                        <div class="col-search-input mt-2 hidden">
                            <input type="text"
                                ...
                                data-column="5" placeholder="Cari customer...">
                        </div>
                    </th>
        ...
        {{-- Modal Create Limit --}}
        <div x-show="$store.trsomtStore.showCreateLimitModal" x-cloak
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-transition>
            <div @click.away="$store.trsomtStore.closeCreateLimitModal()" class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">{{ 'Notifikasi' }}</h3>
                <p class="mb-6">{{ 'Batas membuat data sudah terlampaui' }}</p>
                <div class="flex justify-end">
                    <button type="button" @click="$store.trsomtStore.closeCreateLimitModal()"
...
@push('styles')
    ...
    <style>
        ...
        #returpenjualanorderTable td {
            text-align: left !important;
            vertical-align: middle;
        }

        /* Kolom Aksi: jangan mepet, tapi tetap ringkas */
        #returpenjualanorderTable th:last-child,
        #returpenjualanorderTable td:last-child {
            white-space: nowrap;
            text-align: center;
        }
                ...
                confirmDelete() {
                    ...
                    fetch(this.deleteUrl, {
                        ...
                        .then(result => {
                            ...
                            if (result.ok) {
                                const table = $('#returpenjualanorderTable').DataTable();
                                if (rowToDelete) {
        ...
        $(function() {
            ...
            columns.push({
                ...
                render: function(data, type, row) {
                    ...
                    if (canView) {
                    html += `<a href="returpenjualan/${data}/view">
                    ...
                    if (canEdit) {
                    html += `<a href="returpenjualan/${data}/edit" class="inline-flex items-center bg-yellow-500 text-white px-3 py-1.5 text-xs rounded hover:bg-yellow-600">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                            Edit
                    ...
                    if (canDelete) {
                    let deleteUrl = '{{ route('returpenjualan.index') }}/' + data + '/delete';
            ...
            $('#returpenjualanorderTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                ...
                initComplete: function() {
                    ...
                    $(document).on('click.returpenjualan-column-search', function(e) {
