...
@section('title', $action === 'delete' ? 'Retur Penjualan - Delete' : ($action === 'view' ? 'Retur Penjualan - View' :
...
@section('content')
    ...
    <style>
        ...
        .view-readonly [role="button"],
        .view-readonly button,
        .view-readonly a {
            pointer-events: none;
        }

        .view-readonly .view-allow-interaction {
            pointer-events: auto;
        }
    ...
    <div x-data="itemsTable()" x-init="init()">
                ...
                <div class="p-4">
                    <div class="space-y-2">
                            ...
                            <div x-data="{ showNoItems: false }" x-init="window.addEventListener('returpenjualan-show-no-items', () => { showNoItems = true })"
                                x-show="showNoItems && Number(document.getElementById('itemsCount')?.value || 0) === 0"
        ...
        @else
            <form action="{{ route('returpenjualan.update', $returpenjualan->ftranmtid) }}" method="POST"
                class="{{ $action === 'view' ? 'view-readonly' : '' }}" data-form-draft="true"
                                ...
                                <div x-data="{ autoCode: true }">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <input type="text" name="fsono" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                                            :disabled="autoCode"
                                            :required="!autoCode"
                                            value="{{ strtoupper($returpenjualan->fsono ?? '') }}"
                                            :class="autoCode ? 'bg-gray-100 text-gray-500 cursor-not-allowed border-gray-200' : 'bg-white'"
                                            :placeholder="autoCode ? 'Auto Generated' : 'Wajib diisi'"
                                            oninput="this.value = this.value.toUpperCase()">
                                        <label class="inline-flex items-center select-none cursor-pointer">
                                                            ...
                                                            <div>
                                                                <template x-if="it.units && it.units.length > 1">
                                                                    <select class="w-full border rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500"
                                                                        ...
                                                                        <template x-for="(u, uIdx) in it.units"
                                                                            :key="uIdx">
                                    ...
                                    <div class="flex flex-wrap items-center gap-3 flex-shrink-0">
                                        ...
                                        <div x-data="soFormModal()" class="mt-3">
                                            ...
                                            <div x-show="showHistoryModal" x-cloak
                                                class="fixed inset-0 z-[96] flex items-center justify-center"
                                                x-transition.opacity>
                                                <div class="absolute inset-0 bg-black/50" @click="closeHistory()"></div>
                                                <div
                                                    class="relative bg-white w-[92vw] max-w-4xl rounded-2xl shadow-2xl overflow-hidden">
                                                                ...
                                                                <tbody>
                                                                    <template x-for="(row, idx) in historyRows" :key="idx">
                                                                        <tr class="border-t">
                                                                            ...
                                                                            <td class="p-2 text-right"
                                                                                x-text="row.fqty + ' ' + row.fsatuan"></td>
                                                                            <td class="p-2 text-right"
                                                                                x-text="fmt(row.fprice)"></td>
                                                                            <td class="p-2 text-right"
                                                                                x-text="fmt(row.famount)"></td>
                                                                    ...
                                                                    <tr x-show="!historyRows.length">
                                                                        <td colspan="6"
                                                                            class="p-4 text-center text-gray-500">Tidak ada
                                                                            riwayat.</td>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                ...
                <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                    <div class="p-4 flex items-center justify-end gap-3">
                            ...
                            @elseif ($action === 'view')
                                @php
                                    $isApproved = (int) ($returpenjualan->fapproval ?? 0) === 1;
                                    $isPrinted = ! can_print_again() && (int) ($returpenjualan->fprint ?? 0) === 1;
                                @endphp
                                @if (!$isApproved)
                                    <button type="button"
                                        onclick="Swal.fire({ icon: 'warning', title: 'Informasi', text: 'Retur Penjualan belum di-approve dan tidak boleh dicetak.', confirmButtonColor: '#3b82f6' })"
                                        class="view-allow-interaction inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-700 px-4 text-xs font-semibold text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            ...
                            <button type="button" @click="window.location.href='{{ route('returpenjualan.index') }}'"
                                class="view-allow-interaction inline-flex h-9 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 text-xs font-semibold text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
            ...
            {{-- FOOTER INFO --}}
            @php
                $lastUpdate = $returpenjualan->fupdatedat ?: $returpenjualan->fcreatedat;
                $updatedBy = $returpenjualan->fuserupdate ?: ($returpenjualan->fusercreate ?: '—');
            @endphp
    ...
    @if ($action === 'delete' && $canDeletePermission)
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 allow-action">
                <h3 class="text-lg font-semibold mb-4">Hapus Retur Penjualan ini?</h3>
    ...
    function itemsTable() {
        return {
            ...
            showToast(message, type = 'success') {
                if (window.toast) {
                ...
                } else if (typeof window.showAppErrorAlert === 'function' || typeof window.showAppInfoAlert ===
                    'function' || typeof window.showAppSuccessToast === 'function') {
                    ...
                    else window.showAppSuccessToast(message, {
                        timer: 3000
                    });
                } else {
