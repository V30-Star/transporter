...
@section('title', 'Hapus Group Customer')
...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[700px] mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Group Customer</h2>
            <p class="text-gray-600 mt-1">Data berikut akan dihapus secara permanen:</p>
        </div>

        <div class="bg-gray-50 rounded-lg p-6 space-y-4">
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Kode Group Customer</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900 uppercase">
                    {{ $groupcustomer->fgroupcode }}
                </div>
            </div>

            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Nama Group Customer</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900 uppercase">
                    {{ $groupcustomer->fgroupname }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium
                        {{ $groupcustomer->fnonactive == '1' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ $groupcustomer->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>

            @if($groupcustomer->fcreatedby)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Dibuat</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $groupcustomer->fcreatedby ?? '-' }} -
                    {{ $groupcustomer->fcreatedat ? \Carbon\Carbon::parse($groupcustomer->fcreatedat)->format('d/m/Y H:i') : '-' }}
                </div>
            </div>
            @endif

            @if($groupcustomer->fupdatedat)
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Diupdate</div>
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $groupcustomer->fupdatedby ?? '-' }} -
                    {{ \Carbon\Carbon::parse($groupcustomer->fupdatedat)->format('d/m/Y H:i') }}
                </div>
            </div>
            @endif
        </div>
        ...
        @php
            $hasRelatedData = \Illuminate\Support\Facades\DB::table('mscustomer')
                ->where('fgroup', $groupcustomer->fgroupid)
                ->exists();
        @endphp
        ...
        <div class="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div class="flex items-start">
                ...
                <div>
                    <h4 class="text-sm font-semibold text-red-700">Group Customer Tidak Dapat Dihapus</h4>
                    <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                        <li>Group Customer sudah digunakan dalam data Customer</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated: {{ $groupcustomer->fupdatedat ? \Carbon\Carbon::parse($groupcustomer->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($groupcustomer->fcreatedat)->format('d M Y, H:i') }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus group customer <strong class="text-gray-900">{{ $groupcustomer->fgroupcode }} - {{ $groupcustomer->fgroupname }}</strong>?
                </p>
    ...
    <div x-data="notificationApp()" x-show="showNotification" x-cloak x-transition:enter="transition ease-out duration-300"
        ...
        <div :class="notificationType === 'success' ? 'bg-green-500' : 'bg-red-500'"
            ...
            <button @click="showNotification = false" class="ml-4 text-white hover:text-gray-200">x</button>
        </div>
    </div>
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('groupcustomer.destroy', $groupcustomer->fgroupid) }}', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.success || data.redirect) {
                    setTimeout(() => {
                        window.location.href = '{{ route('groupcustomer.index') }}';
                    }, 1000);
                }
            })
            .catch(error => {
