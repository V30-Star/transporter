                                    ...
                                    <tbody>
                                        <template x-for="(it, i) in savedItems" :key="(it.uid ? (it.uid + '_' + i) : ('item-' + i))">
                                            <tr class="border-t align-top hover:bg-gray-50">
                                                ...
                                                @if ($showMutasiPrices)
                                                    <td class="p-2 text-right">
                                                        <div class="px-2 py-1 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded text-right font-semibold"
                                                            x-text="fmtMoney(it.fprice)"></div>
                                                    </td>
                                                    <td class="p-2 text-right">
                                                        <div class="px-2 py-1 text-sm text-gray-900 bg-gray-50 border border-gray-300 rounded text-right font-semibold"
            ...
            <div class="lg:col-span-4" x-data="{ autoCode: true }">
                ...
                <div class="flex items-center gap-3">
                    <input type="text" name="fstockmtno"
                        ...
                        class="w-full border rounded px-3 py-2 uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                        :disabled="autoCode"
                        :required="!autoCode"
                        ...
                        <div x-data="itemsTable()" x-init="init()" class="space-y-2">
                            ...
                            <div class="overflow-auto border rounded">
                                <table class="mutasi-detail-table min-w-full text-sm balanced-detail-table"
                                    ...
                                    <colgroup>
                                        <col style="width:2%;">
                                         <col style="width:{{ $showMutasiPrices ? '15%' : '18%' }};">
                                         <col style="width:{{ $showMutasiPrices ? ($action === 'view' ? '30%' : '35%') : ($action === 'view' ? '50%' : '40%') }};">
                                         <col style="width:{{ $showMutasiPrices ? '8%' : '12%' }};">
                                         <col style="width:{{ $showMutasiPrices ? '12%' : '18%' }};">
                                         @if ($showMutasiPrices)
                                             <col style="width:12%;">
                                             <col style="width:{{ $action === 'view' ? '18%' : '12%' }};">
                                         @endif
                                         @if ($action !== 'view')
    ...
    @if ($action === 'delete' && $canDeletePermission)
        ...
        <div id="deleteModal"
            ...
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 allow-action">
                <h3 class="text-lg font-semibold mb-4">Hapus Mutasi ini?</h3>
                <form id="deleteForm"
                    action="{{ route('mutasi.destroy', $mutasi->fstockmtid) }}" method="POST">
                    @csrf
                    @method('DELETE')
