...
@section('content')
    <div x-data class="bg-white rounded shadow p-4">
        ...
        <div class="flex justify-end items-center mb-4">
            ...
            @if ($canCreate)
                <a href="{{ route('penerimaanbarang.create') }}"
                    @click="if (@js($createLimitReached)) { $event.preventDefault(); $store.penerimaanbarangStore.openCreateLimitModal(); }"
        ...
        {{-- Modal Create Limit --}}
        <div x-show="$store.penerimaanbarangStore.showCreateLimitModal" x-cloak
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-transition>
            <div @click.away="$store.penerimaanbarangStore.closeCreateLimitModal()" class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">{{ 'Notifikasi' }}</h3>
                <p class="mb-6">{{ 'Batas membuat data sudah terlampaui' }}</p>
                <div class="flex justify-end">
                    <button type="button" @click="$store.penerimaanbarangStore.closeCreateLimitModal()"
...
@push('styles')
    ...
    <style>
        ...
        #penerimaanbarangTable th:last-child,
        #penerimaanbarangTable td:last-child {
            white-space: nowrap;
            text-align: center;
        }
        ...
        #penerimaanbarangTable th,
        #penerimaanbarangTable td {
            text-align: left !important;
            vertical-align: middle;
        }
            ...
            if (hasActions) {
                columns.push({
                    ...
                    render: function(data, type, row) {
                        ...
                        if (canView) {
                            html += `<a href="penerimaanbarang/${data}/view">
                        ...
                        if (canEdit) {
                            html += `<a href="penerimaanbarang/${data}/edit" class="inline-flex items-center bg-yellow-500 text-white px-3 py-1.5 text-xs rounded hover:bg-yellow-600">
                            <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                            Edit
