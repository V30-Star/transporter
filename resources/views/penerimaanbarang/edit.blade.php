                        ...
                        <div x-data="{ autoCode: true }">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="fstockmtno"
                                    ...
                                    value="{{ strtoupper(old('fstockmtno', $displayFstockmtno ?? $penerimaanbarang->fstockmtno ?? '')) }}"
                                    :disabled="autoCode"
                                    :required="!autoCode"
                                        ...
                                        <td class="p-2">
                                            <div class="flex w-full max-w-full">
                                                ...
                                                <button type="button" @click="descItemName = @js($item['fitemname'] ?? ''); descValue = @js($item['fdesc'] ?? ''); showDescModal = true"
                                                    class="shrink-0 inline-flex items-center border border-l-0 rounded-r px-2 py-1 transition-colors {{ !empty($item['fdesc']) ? 'btn-desc-filled font-medium' : 'btn-desc-empty' }}"
                ...
                <div class="flex items-center justify-end gap-3 px-4 py-3 bg-gray-50">
                    ...
                    <div class="flex gap-2">
                        ...
                        @elseif ($action === 'view' && $canPrint)
                            @php
                                $isApproved = (int) ($penerimaanbarang->fapproval ?? 0) === 1;
                                $isPrinted = ! can_print_again() && (int) ($penerimaanbarang->fprint ?? 0) === 1;
                            @endphp
                            @if (!$isApproved)
                                <button type="button"
                                    onclick="Swal.fire({ icon: 'warning', title: 'Informasi', text: 'Penerimaan Barang belum di-approve dan tidak boleh dicetak.', confirmButtonColor: '#3b82f6' })"
                            ...
                            @elseif ($isPrinted)
                                <button type="button"
                                    onclick="Swal.fire({ icon: 'warning', title: 'Informasi', text: 'Penerimaan Barang Sudah Pernah diPrint.', confirmButtonColor: '#3b82f6' })"
                                    class="inline-flex items-center gap-2 px-5 py-2 text-white text-sm font-medium rounded-lg bg-blue-600 hover:bg-blue-700 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m10 0v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5m10 0v5H7v-5">
    ...
    @if ($action === 'delete' && $canDeletePermission)
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 allow-action">
                <h3 class="text-lg font-semibold mb-4">Hapus Penerimaan Barang ini?</h3>
                <form id="deleteForm" action="{{ route('penerimaanbarang.destroy', $penerimaanbarang->fstockmtid) }}" method="POST">
                    @csrf
                    @method('DELETE')
        ...
        <script>
            ...
            function confirmDelete(forceSave = false) {
                ...
                fetch('{{ route('penerimaanbarang.destroy', $penerimaanbarang->fstockmtid) }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(result => {
                        if (result.ok) {
                            ...
                            setTimeout(() => {
                                window.location.href = '{{ route('penerimaanbarang.index') }}';
                            }, 500);
                ...
                submitForm(form) {
                    ...
                    for (const row of this.savedItems) {
                        ...
                        if (seenKeys.has(key)) {
                            ...
                            this.warningMessage = `Kode produk ${code} dengan nomor acak yang sama tidak boleh dobel dalam satu Penerimaan Barang.`;
                            this.warningItems = [];
                            this.warningCanProceed = false;
                            this.pendingSubmitForm = null;
                            this.pendingValidRows = [];
