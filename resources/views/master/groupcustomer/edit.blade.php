...
@section('title', $action === 'delete' ? 'Hapus Group Customer' : 'Edit Group Customer')

@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-2xl mx-auto">
        {{-- HEADER --}}
        <h2 class="text-2xl font-bold mb-6 text-center">
            {{ $action === 'delete' ? 'Data Group Customer' : 'Edit Group Customer' }}
        </h2>
        ...
        <span class="text-sm text-gray-600 flex justify-between items-center">
            ...
            <span>{{ \Carbon\Carbon::parse($groupcustomer->fupdatedat ?: $groupcustomer->fcreatedat)->timezone('Asia/Jakarta')->format('d M Y, H:i:s') }}</span>
        </span>
    </div>
    ...
    @if ($action === 'delete')
        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                ...
                <p class="mb-2 font-medium">Hapus groupcustomer ini?</p>
                <p class="mb-1 text-sm text-gray-600">Kode: <strong>{{ $groupcustomer->fgroupcode }}</strong></p>
                <p class="mb-6 text-sm text-gray-600">Nama: <strong>{{ $groupcustomer->fgroupname }}</strong></p>
