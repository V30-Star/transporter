...
<div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
    ...
    <div class="p-4 grid grid-cols-1 lg:grid-cols-12 gap-3">
        ...
        <div class="lg:col-span-4" x-data="{ autoCode: true }">
            ...
            <div class="flex items-center gap-3">
                <input type="text" name="fstockmtno" class="w-full border rounded px-3 py-2 uppercase"
                    :disabled="autoCode" value="{{ strtoupper(old('fstockmtno', $assembling->fstockmtno ?? '')) }}"
                    :class="autoCode ? 'bg-gray-200 cursor-not-allowed' : 'bg-white'"
                    oninput="this.value = this.value.toUpperCase()">
        ...
        <div class="lg:col-span-12">
            ...
            @enderror
                                    ...
                                    addIfComplete() {
                                        ...
                                        if (!this.isComplete(r)) {
                                            ...
                                            if (!r.fsatuan) return (r.units.length > 1 ? this.$refs.draftUnit?.focus() : this.$refs.draftCode
                                                ?.focus());
                                            if (!(@json(stock_boleh_minus()) ? Number(r.fqty) !== 0 : Number(r.fqty) > 0)) return this.$refs.draftQty?.focus();
                                            return;
                                        }
                                    ...
                                    onSubmit($event) {
                                        ...
                                        if (duplicateCode) {
                                            ...
                                            Swal.fire({
                                                ...
                                                text: `Kode produk ${duplicateCode} dengan nomor acak yang sama tidak boleh dobel dalam satu Assembling.`,
                    ...
                    {{-- FOOTER INFO --}}
                    @php
                        $lastUpdate = $assembling->fupdatedat ?: $assembling->fcreatedat;
                        $updatedBy = $assembling->fuserupdate ?: ($assembling->fusercreate ?: '—');
                    @endphp
