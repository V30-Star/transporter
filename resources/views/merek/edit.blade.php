...
@section('title', 'Edit - Master Merek')
...
<div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-6">
        ...
        <form action="{{ route('merek.update', $merek->fmerekid) }}" method="POST" id="formMerek">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                <div class="flex items-center gap-2 px-4 pt-3 pb-0">
                    ...
                    <p class="text-xs font-bold uppercase tracking-wide text-gray-400">Identitas Merek</p>
                </div>
                <div class="p-4 space-y-3">

                    {{-- Kode & Nama Merek (2 kolom) --}}
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-bold text-gray-600 mb-1">
                                Kode Merek <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="fmerekcode" id="fmerekcode"
                                value="{{ old('fmerekcode', $merek->fmerekcode) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fmerekcode') border-red-400 @enderror"
                                maxlength="10" placeholder="cth. MRK-001">
                            @error('fmerekcode')
                        ...
                        <div>
                            ...
                            <input type="text" name="fmerekname" id="fmerekname"
                                value="{{ old('fmerekname', $merek->fmerekname) }}"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 @error('fmerekname') border-red-400 @enderror"
                                maxlength="50" placeholder="cth. Coca-Cola" autofocus>
                            @error('fmerekname')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden">
                ...
                <div class="p-4 space-y-4">
                    ...
                    {{-- Status Aktif --}}
                    <div x-data="{ active: {{ old('fnonactive', $merek->fnonactive) == '1' ? 'false' : 'true' }} }">
                        <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50 cursor-pointer hover:border-gray-300 transition-colors"
                            ...
                            <div>
                                <p class="text-sm text-gray-800">Merek aktif</p>
                                <p class="text-xs text-gray-400 mt-0.5">Non-aktif menyembuyen merek dari transaksi baru</p>
                            </div>
                ...
                <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t border-gray-200">
                    <button type="button"
                        onclick="window.location.href='{{ route('merek.index') }}'"
        ...
        @php
            ...
            $updatedBy = $merek->fupdatedby ?: ($merek->fcreatedby ?: '—');
        @endphp
