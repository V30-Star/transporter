...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[700px] mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Merek</h2>
            <p class="text-gray-600 mt-1">Data berikut akan dihapus secara permanen:</p>
        </div>

        <div class="bg-gray-50 rounded-lg p-6 space-y-4">
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Kode Merek</div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="text-sm font-medium text-gray-500">Nama Merek</div>
                <div class="col-span-2 text-sm font-semibold text-gray-900 uppercase">
                    {{ $merek->fmerekname }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium
                        {{ $merek->fnonactive == '1' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                        {{ $merek->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $merek->fcreatedby ?? '-' }} - 
                    {{ $merek->fcreatedat ? \Carbon\Carbon::parse($merek->fcreatedat)->format('d/m/Y H:i') : '-' }}
                </div>
            </div>
            @endif
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2 text-sm text-gray-700">
                    {{ $merek->fupdatedby ?? '-' }} - 
                    {{ \Carbon\Carbon::parse($merek->fupdatedat)->format('d/m/Y H:i') }}
                </div>
            </div>
            @endif
        </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated: {{ $merek->fupdatedat ? \Carbon\Carbon::parse($merek->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($merek->fcreatedat)->format('d M Y, H:i') }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus merek <strong class="text-gray-900">{{ $merek->fmerekcode }} - {{ $merek->fmerekname }}</strong>?
                </p>
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('merek.destroy', $merek->fmerekid) }}', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.success || data.redirect) {
                    setTimeout(() => {
                        window.location.href = '{{ route('merek.index') }}';
                    }, 1000);
                }
            })
            .catch(error => {
