...
@section('content')
    <div x-data="{
        ...
        {{-- Search (Live) --}}
        <form id="searchForm" method="GET" action="{{ route('merek.index') }}"
        ...
        <table class="min-w-full border text-sm">
            <thead class="bg-gray-100">
                <tr>
                    <th class="border px-2 py-1 cursor-pointer sortCol" data-sort-by="fmerekcode">
                        <div class="flex items-center gap-1">
                            <span>Kode Merek</span>
                            <span id="icon-fmerekcode" class="text-lg font-semibold text-green-600">⇅</span>
                        </div>
                    </th>
                    <th class="border px-2 py-1 cursor-pointer sortCol" data-sort-by="fmerekname">
                        <div class="flex items-center gap-1">
                            <span>Nama Merek</span>
                            <span id="icon-fmerekname" class="text-lg font-semibold text-green-600">⇅</span>
                        </div>
                    </th>
            ...
            <tbody id="tableBody">
                @forelse ($mereks as $item)
                    <tr class="hover:bg-gray-50">
                        <td class="border px-2 py-1">{{ $item->fmerekcode }}</td>
                        <td class="border px-2 py-1">{{ $item->fmerekname }}</td>
                        @if ($showActionsColumn)
                            <td class="border px-2 py-1 space-x-2">
                                @if ($canEdit)
                                    <a href="{{ route('merek.edit', $item->fmerekid) }}">
                                        <button
                                ...
                                @if ($canDelete)
                                    <button
                                        @click="$dispatch('open-delete', '{{ route('merek.destroy', $item->fmerekid) }}')"
        ...
        <div id="pagination" class="mt-4 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2">
                <button id="prevBtn"
                    class="px-3 py-1 rounded border hover:bg-gray-100 {{ $mereks->onFirstPage() ? 'opacity-50' : '' }}"
                    {{ $mereks->onFirstPage() ? 'disabled' : '' }}
                    data-page="{{ $mereks->previousPageUrl() ?? '' }}">&larr;</button>

                <span id="pageInfo" class="text-sm">
                    Page {{ $mereks->currentPage() }} of {{ $mereks->lastPage() }}
                </span>

                <button id="nextBtn"
                    class="px-3 py-1 rounded border hover:bg-gray-100 {{ $mereks->hasMorePages() ? '' : 'opacity-50' }}"
                    {{ $mereks->hasMorePages() ? '' : 'disabled' }}
                    data-page="{{ $mereks->nextPageUrl() ?? '' }}">&rarr;</button>
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            // state sort awal dari server (fallback ke fmerekid desc)
            const sortState = {
                by: {!! isset($sortBy) ? json_encode($sortBy) : '"fmerekid"' !!},
        ...
        `;
            ...
            function applySortIcons() {
                ['fmerekcode', 'fmerekname'].forEach(col => {
            ...
            function render(json) {
                ...
                if (json.perms) perms = json.perms; // sinkron izin dari server
                ...
                // update sort dari server bila ada
                ...
                // opsional: sinkron URL address bar (tanpa reload)
            ...
            // klik header untuk sort (tanpa reload)
            document.querySelectorAll('.sortCol').forEach(th => {
                th.addEventListener('click', () => {
                    ...
                    if (sortState.by === col) {
                        ...
                        sortState.by = col;
                        sortState.dir = 'asc'; // awalnya asc saat ganti kolom
                    }
                    applySortIcons();
                    fetchTable(buildUrl());
                });
