...
<body>
    ...
    <div id="raw-templates" style="display: none;">
        ...
        <div id="tpl-header">
            <div class="header-row">
                ...
                <div>
                    <div class="title-so">PELUNASAN CUSTOMER</div>
                    <div class="so-no">No. {{ $hdr->fkasmtno ?? '-' }}</div>
                </div>
            </div>

            <div class="customer-container">
                <span class="customer-label">Informasi Pembayaran</span>
                <div style="display: flex; justify-content: space-between; align-items: stretch; gap: 10px;">
                    <div style="flex: 1; padding-right: 10px; border-right: 1px solid #000;">
                        <div style="font-size: 10px;">
                            <strong>Customer:</strong> {{ $hdr->fwhom ?: '-' }}
                        </div>
        ...
        <table id="tpl-table">
            <thead id="tpl-thead">
                <tr>
                    ...
                    <th>No. Faktur / Tgl.</th>
                    <th class="text-right" style="width: 68px;">Nilai Order</th>
                    <th class="text-right" style="width: 68px;">Sisa Piutang</th>
                    <th class="text-right" style="width: 72px;">Nilai Bayar</th>
                </tr>
            </thead>
            <tbody id="raw-rows">
                @forelse ($dt as $i => $row)
                    {{-- Row 1: No & No Faktur + Tanggal --}}
                    <tr class="item-row item-row-main">
                        <td class="text-center row-no">{{ $i + 1 }}</td>
                        <td colspan="4" style="font-weight: bold;">
                            {{ $row->frefno ?? '-' }}
                            @if(!empty($row->tgl_faktur))
                                <span style="font-weight: normal; font-size: 9px; color: #444; margin-left: 6px;">(Tgl: {{ $row->tgl_faktur }})</span>
                            @endif
                        </td>
                    </tr>
                    {{-- Row 2: Nilai Order, Sisa Piutang, Disc, Nilai Bayar --}}
        ...
        <div id="tpl-summary">
            ...
            @php
                ...
                $totalBayarAkhir = (float) ($hdr->famountpay ?? (($totalAmount ?? 0) + $adminBank + $adjustment));
            @endphp
    ...
    <script>
        function runResponsivePagination() {
            ...
            let globalRow = 1;
            document.querySelectorAll('#print-container tbody tr.item-row-main').forEach(tr => {
        ...
        <div id="tpl-header">
            <div class="header-row">
                ...
                <div style="min-width: 260px;">
                    ...
                    <table class="info-table" style="width: 100%;">
                        <tr>
                            <td style="width: 105px;"><strong>{{ 'Tanggal' }}</strong></td>
