...
<head>
    ...
    <style>
        * {
        ...
        .po-header {
            display: grid;
            grid-template-columns: 30mm 40mm 40mm 30mm;
            gap: 5px;
            font-size: 9px;
            padding: 8px 5px;
        }
        ...
        .po-detail {
            display: grid;
            grid-template-columns: 30mm 40mm 20mm 30mm;
            gap: 5px;
            font-size: 8px;
        ...
        @else
            ...
            @foreach ($chunkedData as $pageIndex => $pageData)
                <div class="a4-container">
                    ...
                    @foreach ($pageData as $index => $prh)
                        {{-- PO Header (Parent) - Hitam --}}
                        <div class="po-header">
                            <div>{{ $prh->fprno }}</div>
                            <div>{{ \Carbon\Carbon::parse($prh->fprdate)->format('d/m/Y') }}</div>
                            <div>{{ $prh->supplier_name ?? $prh->fsupplier }}</div>
                            <div>{{ $prh->fket ?? 'LOCO BL' }}</div>
                        </div>

                        {{-- PO Detail Rows (Child) - Merah --}}
                        @if ($prh->details && $prh->details->count() > 0)
