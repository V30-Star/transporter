...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[1800px] w-full mx-auto">
                    ...
                    <div>
                        ...
                        <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                            @foreach ($relatedMessages as $msg)
                                <li>User sudah direferensi di {{ strtolower($msg) }}</li>
                            @endforeach
                        </ul>
                    </div>
                </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated:
                {{ $sysuser->updated_at ? \Carbon\Carbon::parse($sysuser->updated_at)->format('d M Y, H:i') : \Carbon\Carbon::parse($sysuser->created_at)->format('d M Y, H:i') }}
            </p>
        </div>
    </div>
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-gray-700 mb-4">
                        Apakah Anda yakin ingin menghapus user <strong class="text-gray-900">{{ $sysuser->fsysuserid }} -
                            {{ $sysuser->fname }}</strong>?
                    </p>
