...
@section('content')
    <div x-data class="bg-white rounded shadow p-4">
        ...
        <div class="flex justify-end items-center mb-4">
            ...
            @if ($canCreate)
                <a href="{{ route('tr_poh.create') }}"
                    @click="if (@js($createLimitReached)) { $event.preventDefault(); $store.tr_pohStore.openCreateLimitModal(); }"
        ...
        {{-- Modal Create Limit --}}
        <div x-show="$store.tr_pohStore.showCreateLimitModal" x-cloak
            class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-transition>
            <div @click.away="$store.tr_pohStore.closeCreateLimitModal()" class="bg-white rounded-lg shadow-lg max-w-sm w-full p-6">
                <h3 class="text-lg font-semibold mb-4">Notifikasi</h3>
                <p class="mb-6">Batas membuat data sudah terlampaui</p>
                <div class="flex justify-end">
                    <button type="button" @click="$store.tr_pohStore.closeCreateLimitModal()"
        ...
        $(function() {
            ...
            $('#tr_pohTable').DataTable({
                ...
                initComplete: function() {
                    ...
                    $(document).on('click.tr_poh-column-search', function(e) {
