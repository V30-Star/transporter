...
════════════════════════════════════════════════════════════════════ --}}
    ...
    <div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl mb-3 overflow-hidden" x-data="{ showDescModal: false, descItemName: '', descValue: '' }">
                    ...
                    <div class="p-4">
                        <div class="overflow-auto border rounded">
                            <table class="po-detail-table min-w-full text-sm balanced-detail-table"
                                ...
                                <thead class="bg-gray-100">
                                    <tr>
                                        ...
                                        <th class="p-2 text-right w-28 whitespace-nowrap">Total Harga (Rp.)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($savedItems as $idx => $it)
                                        <tr class="border-t align-top hover:bg-gray-50">
                                            <td class="p-2 text-gray-400">{{ $idx + 1 }}</td>
                                            <td class="p-2">
                                                <div class="px-2 py-1 text-sm text-gray-600 bg-gray-50 border rounded font-mono">{{ $it['fitemcode'] }}</div>
                                            </td>
                                            <td class="p-2">
                                            ...
                                            <td class="p-2">
                                                <div class="px-2 py-1 text-sm text-gray-600 bg-gray-50 border rounded">{{ $it['fsatuan'] }}</div>
                                            </td>
                                            <td class="p-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if ($canPrint)
                                ...
                                <a href="{{ route('tr_poh.print', $tr_poh->fpono) }}" target="_blank" rel="noopener"
                                    class="inline-flex items-center gap-2 px-5 py-2 border border-gray-300 {{ $isPrinted ? 'bg-gray-400 text-white pointer-events-none cursor-not-allowed' : 'bg-white text-gray-600 hover:bg-gray-50 hover:border-gray-400' }} text-sm font-medium rounded-lg transition-colors">
                                    <svg class="w-6 h-6 {{ $isPrinted ? 'text-white' : 'text-gray-500' }}" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @else
                                ...
                                <tbody>
                                    <template x-for="(row, i) in rows" :key="row.uid">
                                        <tr class="border-t align-top hover:bg-gray-50">
                                            ...
                                            <td class="p-2">
                                                <div class="px-2 py-1 text-sm text-gray-600 bg-gray-50 border rounded"
                                                    x-text="row.fprno || row.frefdtno || '-'"></div>
                                            </td>
                                            ...
                                            <td class="p-2">
                                                <div class="px-2 py-1 text-sm text-gray-600 bg-gray-50 border rounded text-right"
                                                    x-text="fmtCurr(row.ftotal)"></div>
                                            </td>

                                            {{-- Total Rp --}}
                                            <td class="p-2">
                                                <div class="px-2 py-1 text-sm text-gray-600 bg-gray-50 border rounded text-right"
                                                    x-text="rupiah(itemTotalRp(row.ftotal))"></div>
                                            </td>
            ...
            {{-- FOOTER INFO --}}
            @php
                $lastUpdate = $tr_poh->fupdatedat ?: $tr_poh->fcreatedat;
                $updatedBy = $tr_poh->fuserupdate ?: ($tr_poh->fusercreate ?: '—');
            @endphp
