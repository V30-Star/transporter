...
@section('content')
    ...
    <div x-data="mainForm()" x-init="init()">
        <form action="{{ route('tr_poh.store') }}" method="POST" data-form-draft="true"
            data-draft-key="tr_poh:create" @submit.prevent="submitForm($el)">
            @csrf
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <div x-data="{ autoCode: true }">
                            ...
                            <div class="flex items-center gap-2">
                                <input type="text" name="fpohid" class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                                    :disabled="autoCode"
                                    :required="!autoCode"
                            ...
                            <select name="fcurrency" id="currencySelect" x-model="selectedCurrCode" @change="onCurrencyChange()"
                                ...
                                @foreach ($currencies as $cur)
                                    <option value="{{ $cur->fcurrcode }}" data-id="{{ $cur->fcurrid }}"
                                        data-rate="{{ $cur->frate }}"
                                        {{ old('fcurrency', 'IDR') == $cur->fcurrcode ? 'selected' : '' }}>
            ...
            {{-- ─── CARD 3: Approval & Aksi ────────────────────── --}}
                ...
                <div class="flex items-center justify-end gap-3 px-5 py-2 bg-gray-50 border-t border-gray-200">
                    <button type="button"
                        onclick="window.location.href='{{ route('tr_poh.index') }}'"
...
<script>
    ...
    function mainForm() {
        ...
        return {
            ...
            recalc(row) {
                const qty = Math.max(0, this.parseQty(row.fqty));
