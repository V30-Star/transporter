...
<body>
    ...
    @php
        ...
        $fsubtotal = (float) ($hdr->fsubtotal ?? 0);
        $subtotal = $fsubtotal > 0 ? $fsubtotal : (float) collect($dt)->sum(fn ($r) => (float) ($r->ftotprice ?? 0));
        $ppnAmount = (float) ($hdr->famountpajak ?? 0);
        $ppnPersen = (float) ($hdr->fppnperso ?? ($hdr->fppnpers ?? 11));
        $grandTotal = (float) ($hdr->famountpo ?? ($hdr->famount ?? ($subtotal + $ppnAmount)));
    @endphp
                    ...
                    <div style="width: 50%; box-sizing: border-box; padding-left: 6px;">
                        <table class="info-table" style="margin-top: 0; width: 100%;">
                            <tr>
                                ...
                                <td>{{ $fmt($hdr->fpodate) }}</td>
                            </tr>
                            @if(!empty($hdr->fbranchcode))
                            <tr>
                                <td>Cabang</td>
                                <td>:</td>
                                <td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $hdr->fbranchcode }}</td>
                            </tr>
                            @endif
                            <tr>
                                <td>Hal</td>
            ...
            <tbody id="raw-rows">
                @foreach ($dt as $i => $r)
                    ...
                    <tr>
                        ...
                        <td class="text-right" style="width: 30%; color: #000; font-weight: bold;" colspan="3">{{ number_format((float) ($r->famount ?? 0), 0, ',', '.') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div id="tpl-summary">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-top: 2px; gap: 4px;">
                ...
                <div style="flex: 0 0 26%; margin-top: 2px;">
                    ...
                    <div style="margin-top: 3px; font-size: 9.5px; line-height: 1.15; border: 1px dashed #000; padding: 2px 3px;">
                        Ket: {{ $hdr->fket }}
                    </div>
                    @endif
                </div>
                ...
                <div style="flex: 0 0 50%;">
                    <table style="width: 100%; border-collapse: collapse; font-size: 12px; font-weight: bold;">
                        <tr>
                            ...
                            <td style="text-align: right; padding: 1px 0;">{{ number_format($subtotal, 0, ',', '.') }}</td>
                        </tr>
                        @if($ppnAmount > 0)
                        <tr>
                        ...
                        <tr class="gt" style="font-weight: bold; font-size: 12.5px;">
                            ...
                            <td style="border-top: 1px solid #000; border-bottom: 1px solid #000; text-align: right; padding: 1.5px 0;">{{ number_format($grandTotal, 0, ',', '.') }}</td>
                        </tr>
                    </table>
    ...
    <div id="raw-templates" style="display: none;">
            ...
            <tbody id="raw-rows">
                @foreach ($dt as $i => $r)
                    <tr class="item-row">
                        ...
                        <td>{{ !empty(trim((string) ($r->fdesc ?? ''))) ? $r->fdesc : (format_product_name($r->product_name ?? '', $r->fspecification ?? $r->product_specification ?? '') ?: '-') }}</td>
                        <td class="text-right">{{ number_format($r->fqty ?? 0, 2, ',', '.') }}
                            {{ $r->funit ?? ($r->fsatuan ?? '') }}</td>
        ...
        <div id="tpl-summary">
            ...
            <div class="terbilang-box">
                Terbilang : <br>
                # {{ strtoupper(terbilang($grandTotal)) }} {{ ($hdr->fcurrency ?? 'IDR') == 'IDR' ? 'RUPIAH' : strtoupper($hdr->fcurrency) }} #
            </div>
