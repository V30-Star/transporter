...
@section('title', 'Listing Penerimaan Barang')
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
                <div class="flex justify-between items-center border-b pb-4 mb-4">
                    <h3 class="text-xl font-bold text-gray-800">Penerimaan Barang</h3>
                    <button onclick="toggleModal(false)"
                        class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
                </div>

                <form method="GET" action="{{ route('listingpenerimaanbarang.print') }}" target="_blank">
                    <div class="space-y-4">
                        {{-- Cabang checkboxes --}}
                        <div>
                        ...
                        <div>
                            ...
                            <select name="warehouse" class="select2 w-full">
                                ...
                                @foreach ($warehouses as $w)
                                    <option value="{{ $w->fwhid }}">{{ $w->fwhcode }} - {{ $w->fwhname }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="block text-xs font-bold uppercase mb-1">Supplier From</label>
                            ...
                            <div><label class="block text-xs font-bold uppercase mb-1">Supplier To</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="block text-xs font-bold uppercase mb-1">Status Tagihan</label>
                                <select name="status" class="w-full border rounded px-3 py-2 text-sm">
                                    <option value="0">Semua</option>
                                    <option value="1">Hanya Sudah Di Tagih</option>
                                    <option value="2">Hanya Belum Di Tagih</option>
                                </select>
                            </div>
                            <div><label class="block text-xs font-bold uppercase mb-1">Urutan</label>
                                <select name="sort_by" class="w-full border rounded px-3 py-2 text-sm">
                                    <option value="no">No. Transaksi</option>
                                    <option value="name">Tanggal</option>
                                </select>
                            </div>
                        </div>

                        <div class="flex gap-4 bg-gray-50 p-3 rounded border">
                            <label class="flex items-center text-sm font-semibold"><input type="checkbox" name="is_detail"
                                    checked class="mr-2"> Detail</label>
                            <label class="flex items-center text-sm font-semibold"><input type="checkbox" name="is_rekap"
                                    class="mr-2"> Rekap</label>
                        </div>
                    </div>
