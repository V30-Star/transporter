...
<head>
    ...
    <title>Rekap Penjualan Customer Per Produk</title>
    <!-- Fonts -->
    ...
    <style>
        * {
        ...
        .sales-header {
            display: grid;
            grid-template-columns: 25mm 90mm 35mm 35mm 82mm;
            gap: 1px;
            font-size: 8.5px;
            padding: 3px 8px;
            align-items: center;
...
<body>
    <div class="no-print">
        ...
        {{-- Excel Download --}}
        <a href="{{ route('reportingrekappenjualancustomerproduk.excel', request()->query()) }}"
    ...
    <div id="raw-source" style="display: none;">
        ...
        <div class="sales-header-labels">
            <div>Produk#</div>
            <div>Nama Produk</div>
            <div class="text-right">Qty.Besar</div>
            <div class="text-right">Qty.Kecil</div>
        ...
        @foreach ($rows->groupBy('fcustno') as $custNo => $customerRows)
            @php
                ...
                $customerQtyBesar = 0;
                $customerQtyKecil = 0;
                $custName = $customerRows->first()->customer_name ?? '';
            @endphp
            <div class="journal-block">
                ...
                <div class="subtotal-row" style="background-color: #f1f5f9; color: #000000;">
                    <div></div>
                    <div>TOTAL {{ $custName }}</div>
                    <div>{{ number_format($customerQtyBesar, 2, ',', '.') }}</div>
                    <div>{{ number_format($customerQtyKecil, 2, ',', '.') }}</div>
                    <div>{{ number_format($customerTotal, 2, ',', '.') }}</div>
                </div>

                @php $grandTotalNota += $customerTotal; @endphp
            </div>
        @endforeach
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        journals.forEach((journal) => {
            ...
            const custHeader = journal.querySelector(".customer-header-row");
            const groupHeaders = Array.from(journal.querySelectorAll(".group-header-row"));
            const poDetails = Array.from(journal.querySelectorAll(".sales-detail"));
            const subtotals = Array.from(journal.querySelectorAll(".subtotal-row"));
