...
@section('title', 'Rekap Penjualan Customer Per Produk')
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-2xl max-w-3xl w-full p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-4">
                <h3 class="text-xl font-bold text-gray-800">Laporan Rekap Penjualan Customer Per Produk</h3>
                <button onclick="toggleModal(false)" class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
            </div>

            <form id="reportForm" method="GET" action="{{ route('reportingrekappenjualancustomerproduk.print') }}" target="_blank">
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            ...
                            <label class="mr-4 text-sm"><input type="radio" name="report_type" value="DETAIL" checked> Detail</label>
                            <label class="text-sm"><input type="radio" name="report_type" value="REKAP"> Rekap</label>
                        </div>
                    </div>
                </div>

                <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
                    <button type="submit" onclick="setAction('{{ route('reportingrekappenjualancustomerproduk.print') }}')" class="px-5 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">Preview</button>
                    <button type="submit" onclick="setAction('{{ route('reportingrekappenjualancustomerproduk.print') }}')" class="px-5 py-2 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-900">Print / PDF</button>
                    <button type="submit" onclick="setAction('{{ route('reportingrekappenjualancustomerproduk.excel') }}')" class="px-5 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700">Export Excel</button>
