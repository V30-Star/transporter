    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-4">
                <h3 class="text-xl font-bold text-gray-800">Analisa Umur Hutang</h3>
                <button onclick="toggleModal(false)" class="text-gray-400 hover:text-gray-600 text-2xl font-bold">&times;</button>
            </div>

            <form method="GET" action="{{ route('analisaumurhutang.print') }}" target="_blank">
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase mb-1">Supplier</label>
                    ...
                    <div class="grid grid-cols-2 gap-2 bg-gray-50 p-3 rounded border">
                        <label class="flex items-center text-sm font-semibold cursor-pointer">
                            <input type="radio" name="mode" value="all" checked onchange="toggleDueDate()" class="mr-2 w-4 h-4 text-blue-600 focus:ring-blue-500"> Semua
                        </label>
                        <label class="flex items-center text-sm font-semibold cursor-pointer">
                            <input type="radio" name="mode" value="due" onchange="toggleDueDate()" class="mr-2 w-4 h-4 text-blue-600 focus:ring-blue-500"> Tgl. Jatuh Tempo s/d
                        </label>
                    </div>

                    <div>
                        <input type="date" name="due_date_to" id="due_date_to" value="{{ date('Y-m-d') }}" class="w-full border rounded px-3 py-2 text-sm" disabled>
                    </div>
                </div>
...
<script>
    ...
    function toggleDueDate() {
        document.getElementById('due_date_to').disabled = document.querySelector('[name="mode"]:checked')?.value !== 'due';
    }
</script>
@endsection
