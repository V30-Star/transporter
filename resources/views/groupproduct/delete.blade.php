...
@section('title', 'Hapus Group Product')
...
@section('content')
    <div class="bg-white rounded shadow p-6 md:p-8 max-w-[700px] mx-auto">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Konfirmasi Hapus Group Product</h2>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="col-span-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium
                        ...
                        {{ $groupproduct->fnonactive == '1' ? 'Non Active' : 'Active' }}
                    </span>
                </div>
            </div>
        ...
        @php
            $hasRelatedData = \Illuminate\Support\Facades\DB::table('msprd')
                ->where('fgroupcode', $groupproduct->fgroupcode)
                ->exists();
        @endphp
        ...
        <div class="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div class="flex items-start">
                ...
                <div>
                    <h4 class="text-sm font-semibold text-red-700">Group Product Tidak Dapat Dihapus</h4>
                    <ul class="mt-2 text-sm text-red-600 list-disc list-inside space-y-1">
                        <li>Group Product sudah digunakan dalam data Product</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Last updated: {{ $groupproduct->fupdatedat ? \Carbon\Carbon::parse($groupproduct->fupdatedat)->format('d M Y, H:i') : \Carbon\Carbon::parse($groupproduct->fcreatedat)->format('d M Y, H:i') }}</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-4">
                    Apakah Anda yakin ingin menghapus group product <strong class="text-gray-900">{{ $groupproduct->fgroupcode }} - {{ $groupproduct->fgroupname }}</strong>?
                </p>
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch('{{ route('groupproduct.destroy', $groupproduct->fgroupid) }}', {
                method: 'POST',
                headers: {
